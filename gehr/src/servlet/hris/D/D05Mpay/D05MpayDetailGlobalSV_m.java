...
/*   Program ID   		: D05MpayDetailSV_m.java                           
/*   Description  		: 개인의 월급여내역 정보를 jsp로 넘겨주는 class   =China, USA         
/*   Note         		:                                                            
...
public class D05MpayDetailGlobalSV_m extends EHRBaseServlet {
	...
	protected void performTask(HttpServletRequest req, HttpServletResponse res)
		...
		try {
            ...
            if(!"X".equals(user_m.e_mss)) {
                ...
                req.setAttribute("servlet", "hris.D.D05Mpay.D05MpayDetailGlobalSV_m");
            	
            	 printJspPage(req, res, WebUtil.JspURL+"common/MSS_SearchPernNameOrg.jsp");
            	 return;
            }

			if (user_m.area.equals(Area.PL) || user_m.area.equals(Area.DE)) { // PL 폴랜드, DE 독일 은 유럽화면으로 
				printJspPage(req, res, WebUtil.ServletURL+ "hris.D.D05Mpay.D05MpayDetailEurpSV_m");
...
//					seqnr = rfc_paid.getLatestPaid3(user_m.empNo, user_m.webUserId); // 5월 21일 추가
					...
					if (yyyy == null || yyyy.equals("")) {
						yymm = year + month;
						//Logger.debug.println(this, "year = " + year + "month = " + month);
					} else {
						yymm = yyyy + mm;
					...
					Logger.info.println(this, "지급내역size : "+ d05MpayDetailData5_vt.size());
					Logger.info.println(this, "지급내역text : "+ d05MpayDetailData5_vt.toString());
					Logger.info.println(this, "해외내역수정 : "+ d05MpayDetailData6_vt.toString());
					Logger.info.println(this, "해외급여 반영내역(항목) 내역 : "+ d05MpayDetailData1_vt.toString());
					Logger.info.println(this, "지급내역/공제내역  : "+ d05MpayDetailData2_vt.toString());
					Logger.info.println(this, "과세추가내역2 : "+ d05MpayDetailData3_vt.toString());
					Logger.info.println(this, "급여명세표 - 개인정보/환율 내역  : "+ d05MpayDetailData4.toString());
					Logger.info.println(this, "지급내역/공제내역 합  : "+ d05MpayDetailData5.toString());
				} // if ( user_m != null ) end
				...
				if (user_m != null) {
					...
					Logger.info.println(this, "선택한 년도 : " + year + "선택한 월:" + month + "선택한 임금유형:" + ocrsn + "yymm;" + yymm + "seqnr :" + seqnr);
					Logger.info.println(this, "급여사유 코드와 TEXT : " + d05ZocrsnTextData_vt.toString());
				...
				req.setAttribute("print_page_name",
										WebUtil.ServletURL
												+ "hris.D.D05Mpay.D05MpayDetailSV_m?jobid_m=kubya_m&year1="
												+ year1 + "&month1=" + month1
				...
				Logger.info.println(this, "선택한 년도 : " + year + "선택한 월:" + month + "선택한 임금유형:" + ocrsn + "yymm;" + yymm + "순번:" + seqnr);
				...
				Logger.info.println(this, "user.empNo:" + user_m.empNo  + "  년도 : " + yymmdd + "선택한 임금유형:" + ocrsn + "flag;" + flag + "seqnr:" + seqnr);
				...
				d05MpayDetailData4 = (D05MpayDetailData4) rfc.getPerson(user_m.empNo, yymmdd, ocrsn, flag, seqnr); // 5월 21일 순번 추가
				d05MpayDetailData5 = (D05MpayDetailData5) rfc.getPaysum(user_m.empNo, yymmdd, ocrsn, flag, seqnr); // 5월 21일 순번 추가
				...
				if (user_m != null) {
					...
					Logger.info.println(this, "선택한 년도 : " + year + "선택한 월:" + month + "선택한 임금유형:" + ocrsn + "yymm;" + yymm);
					Logger.info.println(this, "급여사유 코드와 TEXT : " + d05ZocrsnTextData_vt.toString());
					...
					Logger.info.println(this, "과세추가내역1 : " + d05MpayDetailData3_vt.toString());
					...
					Logger.info.println(this, "변형 과세추가내역 : " + d05MpayDetailData4_vt.toString());

					d05MpayDetailData4 = (D05MpayDetailData4) rfc.getPerson(mapPernr, yymmdd, ocrsn, flag, seqnr); // 5월 21일 순번 추가
					d05MpayDetailData5 = (D05MpayDetailData5) rfc.getPaysum(mapPernr, yymmdd, ocrsn, flag, seqnr); // 5월 21일 순번 추가
					...
					Logger.info.println(this, "해외내역수정 : "+ d05MpayDetailData6_vt.toString());
					Logger.info.println(this, "해외급여 반영내역(항목) 내역 : " + d05MpayDetailData1_vt.toString());
					Logger.info.println(this, "지급내역/공제내역  : " + d05MpayDetailData1_vt.toString());
					Logger.info.println(this, "과세추가내역 : " + d05MpayDetailData1_vt.toString());
					Logger.info.println(this, "급여명세표 - 개인정보/환율 내역  : " + d05MpayDetailData4.toString());
					Logger.info.println(this, "지급내역/공제내역 합  : " + d05MpayDetailData5.toString());
