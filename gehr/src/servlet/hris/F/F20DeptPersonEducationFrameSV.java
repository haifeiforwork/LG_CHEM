...
*   2Depth Name  : 인원현황
*   Program Name :  부서 인사교육현황 frame 호출
*   Program ID   : F20DeptPersonEducationFrameSV
*   Description  : 부서 인사교육현황 각각의 상세화면 조회를 위한 서블릿
*   Note         : 없음
*   Creation     :
*   Update       :
*
...
public class F20DeptPersonEducationFrameSV extends EHRBaseServlet {
	protected void performTask(HttpServletRequest req, HttpServletResponse res) throws GeneralException {
        WebUserData user = WebUtil.getSessionUser(req);

        if(process(req, res, user, "E"))
            printJspPage(req, res, WebUtil.JspURL+"F/F20DeptPersonEducationFrame.jsp");
	}
