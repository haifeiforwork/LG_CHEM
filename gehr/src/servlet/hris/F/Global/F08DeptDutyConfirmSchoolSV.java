...
import hris.F.rfc.Global.F08DeptDutyConfirmSchoolRFC;
...
/**
 * F08DeptDutyConfirmSchoolSV 부서에 따른 직무별/인정학력별 정보를 가져오는
...
public class F08DeptDutyConfirmSchoolSV extends EHRBaseServlet {
	...
	protected void performTask(HttpServletRequest req, HttpServletResponse res)
		...
		try {
			...
			F08DeptDutyConfirmSchoolRFC func = null;
			...
			if (!deptId.equals("")) {
				...
				func = new F08DeptDutyConfirmSchoolRFC();
				ret = func.getDeptDutyConfirmSchool(deptId, checkYN);
			...
			//if (E_RETURN != null && E_RETURN.equals("S")) {
				...
				req.setAttribute("DeptDutySchoolTitle_vt",
						result);
				// req.setAttribute("DeptDutySchoolNote_vt",
				// DeptDutySchoolNote_vt);
				req.setAttribute("meta", meta);

				if (excelDown.equals("ED")) // 엑셀저장일 경우.
					dest = WebUtil.JspURL
							+ "F/F08DeptDutyConfirmSchoolExcel_Global.jsp";
				else
					dest = WebUtil.JspURL + "F/F08DeptDutyConfirmSchool_Global.jsp";
			...
			/*} else {
				...
				String url = "location.href = '" + WebUtil.JspURL
						+ "F/F08DeptDutyConfirmSchool.jsp?checkYn=" + checkYN
						+ "';";
