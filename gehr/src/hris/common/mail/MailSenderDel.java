 ...
 */
public class MailSenderDel extends Thread {

    public MailSenderDel() {
    ...
    public void send(MailData maildata) {
        try{
            ...
            Logger.debug.println(this, "### maildata.AppLineData_vt : " + maildata.AppLineData_vt);
            ...
            for (int i =0 ; i < maildata.AppLineData_vt.size() ; i++) {
                ...
                if(appline.APPL_APPR_SEQN.equals("01")&&appline.APPL_APPU_TYPE.equals("01")) {
                    ...
                    Logger.debug.println(this, "### maildata.phonenumdata : " + phonenumdata);

                    String mailfrom = MAILFROM;
                    String mailto   = MAILTO.equalsIgnoreCase("Yes") ? phonenumdata.E_MAIL : MAILTO;
                    String subject  = "["+maildata.upmuname.trim()+" 취소]"+maildata.user.ename.trim()+"님의 "+ maildata.upmuname.trim() +"에 대한 취소안내";
                    // 2004.09.22 LG화학메일에서 text 형식을 제대로 표현하지 못해서 html code 형식으로 메일 보냄.
                    //String content  = "     "+maildata.user.ename.trim()+"(사번:"+maildata.user.empNo.trim()+")님이 "+ maildata.upmuname.trim() +"을 취소하셨습니다.";
                    String content  = "<table><tr><td width=17>&nbsp;</td><td>"+maildata.user.ename.trim()+"(사번:"+maildata.user.empNo.trim()+")님이 "+ maildata.upmuname.trim() +"을 취소하셨습니다.</td></tr></table>";
