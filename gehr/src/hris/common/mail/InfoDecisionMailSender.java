 ...
 */
public class InfoDecisionMailSender extends Thread {

    public InfoDecisionMailSender() {
    ...
    public void send(MailData maildata) throws GeneralException {
        ...
        try{
            ...
            String subject  = "["+maildata.upmuname.trim()+"]"+maildata.data.ENAME.trim()+"님의 "+ maildata.upmuname.trim() +"에 대한 결재완료 안내";

            // 2004.09.22 LG화학메일에서 text 형식을 제대로 표현하지 못해서 html code 형식으로 메일 보냄.
            //String content  = "     "+maildata.data.ENAME.trim()+"(사번:"+maildata.data.PERNR.trim()+")님의 "+ maildata.upmuname.trim() +"에 대한 결재가 완료되었습니다.\r\n     ESS의 결재진행현황에서 내용을 확인해주시기 바랍니다.";
            String content  = "<table><tr><td width=17>&nbsp;</td><td>"+maildata.data.ENAME.trim()+"(사번:"+maildata.data.PERNR.trim()+")님의 "+ maildata.upmuname.trim() +"에 대한 결재가 완료되었습니다.</td></tr><tr><td width=17>&nbsp;</td><td>ESS의 결재진행현황에서 내용을 확인해주시기 바랍니다.</a></td></tr></table>";
