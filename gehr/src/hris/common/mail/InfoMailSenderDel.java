...
/**
 * InfoMailSenderDel.java
 * 인포멀간사에게 JavaMail 패키지를 사용하여 메일을 보낼수 있는 Class
 *
 * @author 이형석
 * @version 1.0, 2001/12/13
 */
public class InfoMailSenderDel extends Thread {

    public InfoMailSenderDel() {
    ...
    public void send(MailData maildata) throws GeneralException {
        ...
        try{
            ...
            String subject  = "[인포멀 "+maildata.upmuname.trim()+" 취소]"+maildata.user.ename.trim()+"님의 인포멀 "+ maildata.upmuname.trim() +"에 대한 취소안내";
            // 2004.09.22 LG화학메일에서 text 형식을 제대로 표현하지 못해서 html code 형식으로 메일 보냄.
            //String content  = "       &nbsp;&nbsp;&nbsp;"+maildata.user.ename.trim()+"(사번:"+maildata.user.empNo.trim()+" 소속:"+maildata.user.e_orgtx.trim()+")님이 "+ maildata.stext.trim() +" "+ maildata.upmuname.trim() +"을 취소하셨습니다.       ";
            String content  = "<table><tr><td width=17>&nbsp;</td><td>"+maildata.user.ename.trim()+"(사번:"+maildata.user.empNo.trim()+" / 소속:"+maildata.user.e_orgtx.trim()+")님이 "+ maildata.stext.trim() +" "+ maildata.upmuname.trim() +"을 취소하셨습니다.</td></tr></table>";
