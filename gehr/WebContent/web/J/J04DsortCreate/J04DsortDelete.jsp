...
//  Header Stext - Function Name, Objective Name, 대분류 Name
...
//    Vector                 j04Object_vt  = new Vector();
...
//        J01JobMatrixData data = new J01JobMatrixData();

    String OTYPE = request.getParameter("OTYPE_D");
    String OBJID = request.getParameter("OBJID_D");
    String STEXT = request.getParameter("STEXT_D");
//        data.SCLAS = request.getParameter("SCLAS_D");
//        data.SOBID = request.getParameter("SOBID_D");
//        data.BEGDA = request.getParameter("BEGDA_D");

//        j04Object_vt.addElement(data);
//    }
    
    if( !jobid.equals("first") ) {
//      삭제할 대분류와 하위 Job들의 List를 읽는다.
//        for( int i = 0 ; i < l_count ; i++ ) {
//            J01JobMatrixData data = (J01JobMatrixData)j04Object_vt.get(i);

//            if( i == 0 ) {
//              대분류의 기간한정
                ret = rfc.Delete( OTYPE, OBJID, i_begda, user.empNo );
...
//                if( !E_SUBRC.equals("0") || count_E > 0 ) { break; }
//            }
//          대분류가 포함하는 Job의 기간한정
//            ret = rfc.Delete( data.SCLAS, data.SOBID, i_begda, user.empNo );
...
//  한계결정 시 시작일 보다 이전 날짜로 한계결정이 되는 경우에는 다음의 error 메시지를 display 한다.
    ...
    if(diff < 0){
      alert("대분류 시작일의 이전 날짜로 한계결정이 될 수는 없습니다.\n인사담당자에게 문의하여 처리하시기 바랍니다."); 
      return;
    }
  } 
  ...
  if( confirm("대분류와 관련된 모든 데이터가 삭제됩니다.\n삭제하시겠습니까?") ){
...
<form name="form1" method="post" action="">
  ...
  <input type="hidden" name="OTYPE_D" value="<%= OTYPE %>">
  <input type="hidden" name="OBJID_D" value="<%= OBJID %>">
  <input type="hidden" name="STEXT_D" value="<%= STEXT %>">

<!--  <input type="hidden" name="count"   value=" j04Object_vt.size() %>">     <!-- 삭제할 대분류와 Job List count -->
...
//    for( int i = 0 ; i < l_count ; i++ ) {
//        J01JobMatrixData data = (J01JobMatrixData)j04Object_vt.get(i);
//        if( i == 0 ) { STEXT_D = data.STEXT; }
%>
<!--  <input type="hidden" name="OTYPE_D i %>" value=" data.OTYPE %>">
  <input type="hidden" name="OBJID_D i %>" value=" data.OBJID %>">
  <input type="hidden" name="STEXT_D i %>" value=" data.STEXT %>">
  <input type="hidden" name="SCLAS_D i %>" value=" data.SCLAS %>">
  <input type="hidden" name="SOBID_D i %>" value=" data.SOBID %>">
  <input type="hidden" name="BEGDA_D i %>" value=" data.BEGDA %>"> -->
<%
//    }
%>
    ...
    <td valign=top align=center>
     <br>
      <table cellpadding=0 cellspacing=0 border=0 width=460>
        <tr height=30>
          <td colspan=4 class=subt01><img src="<%= WebUtil.ImageURL %>jms/bullet_Dround.gif" align=absmiddle>대분류 삭제</td>
        </tr>
        ...
        <tr>
          <td class=ct align=center>대분류 명</td>
          <td width=1 bgcolor=#999999><img src="<%= WebUtil.ImageURL %>jms/spacer.gif" width=1 height=1></td>
