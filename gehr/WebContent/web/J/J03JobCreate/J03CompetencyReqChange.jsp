...
function changeLine(type){
   ...
   var count = 0;
    ...
    document.form1.action      = "<%= WebUtil.ServletURL %>hris.J.J03JobCreate.J03CompetencyReqChangeSV";
...
//적용일자 필수
  if( checkNull(document.form1.BEGDA_C, "적용일자를") == false ) {
    return;
  } else {
//  Job 시작일 보다 이전 날짜로 적용일자를 설정할수 없다. - error 메시지를 display 한다.
    var beg_date = "<%= E_BEGDA %>";
    var end_date = removePoint(document.form1.BEGDA_C.value);

    diff = dayDiff(addSlash(beg_date), addSlash(end_date));
    if(diff < 0){
      document.form1.BEGDA_C.value = "<%= WebUtil.printDate(E_BEGDA) %>";
...
//요구수준이 선택 안된 데이터 존재하는지 체크 
  ...
  document.form1.BEGDA.value = removePoint(document.form1.BEGDA_C.value);
   ...
   <tr>
     ...
     <td width=746 valign=top align=center>
       <table cellpadding=0 cellspacing=0 border=0 width=746>
         <tr height=26>
           <td colspan=11 class=subt01><img src="<%= WebUtil.ImageURL %>jms/bullet_Dround.gif" align=absmiddle>Competency Requirements 수정</td>
         </tr>
        ...
        <tr>
          ...
          <td class=cc colspan=9> 
            <table cellpadding=0 cellspacing=0>
              <tr>
                <td width=85><input type="text" name="BEGDA_C" value="<%= (J03Q_vt.size() > 0) ? WebUtil.printDate(i_begda) : WebUtil.printDate(E_BEGDA) %>" size="10" onBlur="javascript:dateFormat(this);"></td>
                <td><a href="javascript:fn_openCal('BEGDA_C');"><img src="<%= WebUtil.ImageURL %>jms/btn_searchs.gif" border=0 alt='달력보기'></td>
              </tr>
            </table> 
          </td>
        </tr>
...
%>                   
              <option value='<%= j %>' <%= t_selected == j ? "selected" : "" %>><%= data_L.PSTEXT %></option> 
<%            
          }
%>                        
             </select>
        ...
        <tr height=30 valign=bottom>
          <td colspan=11 align=center>
            <a href="javascript:saveObject();"><img src="<%= WebUtil.ImageURL %>jms/btn_save.gif" border=0 hspace=5 alt="저 장"></a>
      	    <a href="javascript:goMenu('<%= i_sobid %>','2');"><img src="<%= WebUtil.ImageURL %>jms/btn_cancel.gif" border=0 hspace=5 alt="취 소"></a>
          </td>
        </tr>
      </table>
    </td>
