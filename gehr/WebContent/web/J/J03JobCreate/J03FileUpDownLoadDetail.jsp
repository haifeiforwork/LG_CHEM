...
//  이미지 파일이 없는경우 PPT파일이 등록되었는지 check하여 메시지를 달리한다.
    String i_PPT          = (String)request.getAttribute("i_PPT");
...
//PageUtil 관련 script - page처리시 반드시 써준다...
function get_Page(){
    document.form3.action       = "<%= WebUtil.ServletURL %>hris.J.J03JobCreate.J03FileUpDownLoadDetailSV";
  ...
  <tr>
    ...
    <td width=746 valign=top>
      <table cellpadding=0 cellspacing=0 border=0 width=746>
        <tr height=26>
          <td colspan=3 class=subt01><img src="<%= WebUtil.ImageURL %>jms/bullet_Dround.gif" align=absmiddle><%= i_imgidx.equals("3") ? "Job Unit별 KSEA 조회" : "Job Process 조회" %></td>
        </tr>
...
<%
        if( i_PPT.equals("Y") ) {
%>
                <%= i_imgidx.equals("3") ? "Job Unit별 KSEA" : "Job Process" %>에 대한 이미지 파일이 등록되지 않았습니다.
                &nbsp;&nbsp;인사담당자에게 확인해 주시기 바랍니다.
<%
        } else {
%>
                <%= i_imgidx.equals("3") ? "Job Unit별 KSEA" : "Job Process" %>에 대한 파일이 존재하지 않습니다.
                &nbsp;&nbsp;수정화면으로 들어가셔서 작성해 주시기 바랍니다.
<%
        }
%>
                </td>
...
%>
        <tr height=30 valign=bottom>
          <td colspan=11 align=center>
            <a href="javascript:goChange('<%= i_sobid %>','<%= i_imgidx %>');"><img src="<%= WebUtil.ImageURL %>jms/btn_modify.gif" border=0 hspace=5 alt="수 정"></a>
          </td>
        </tr>
      </table>
    </td>
