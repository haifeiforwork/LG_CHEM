...
function goUpLoad() {        //idx == 3이면 KSEA, idx == 4이면 Process
    ...
    document.form1.action = "<%= WebUtil.JspURL %>J/J03JobCreate/J03FileUpLoadPopupFrame.jsp?jobidPop=build";
  ...
  <tr>
    ...
    <td width=746 valign=top align=center>
      <table cellpadding=0 cellspacing=0 border=0 width=746>
        <tr height=26>
          <td colspan=3 class=subt01><img src="<%= WebUtil.ImageURL %>jms/bullet_Dround.gif" align=absmiddle><%= i_imgidx.equals("3") ? "Job Unit별 KSEA 생성" : "Job Process 생성" %></td>
        </tr>
          ...
          <td colspan=3 bgcolor=#efefef>
            <table align=center>
              <tr valign=top>
                ...
                <td width=600>
                 양식을 download받아 <%= i_imgidx.equals("3") ? "Job Unit별 KSEA" : "Job Process" %>를 작성하신 후 upload해 주시기 바랍니다.<br>
                 Upload 후 인사팀에게 "적용일자"와 "Job ID"를 메일로 보내 주시면 반영해 드리도록 하겠습니다.
                </td>
              </tr>
              <tr height=40>
...
%>
                 <a href="javascript:fnDownload('IMGIDX=<%= i_imgidx %>&SOBID=<%= i_sobid %>&OBJID=<%= i_objid %>');"><img src="<%= WebUtil.ImageURL %>jms/btn_download.gif" border=0 hspace=5 alt="양식 Download"></a>
<%
    }
%>
                 <a href="javascript:goUpLoad();"><img src="<%= WebUtil.ImageURL %>jms/<%= i_imgidx.equals("3") ? "btn_KSEAupload.gif" : "btn_processUpload.gif" %>" border=0 hspace=5 alt="<%= i_imgidx.equals("3") ? "KSEA Upload" : "Process Upload" %>"></a>
                </td>
              </tr>
            </table>
          </td>
        ...
        <tr height=30 valign=bottom>
          <td colspan=3 align=center>
           <a href="<%= i_imgidx.equals("3") ? "javascript:goJobProcess();" : "javascript:goMenu('" + i_sobid + "','5');" %>"><img src="<%= WebUtil.ImageURL %>jms/btn_next.gif" border=0 hspace=5 alt="다 음"></a>
          </td>
        </tr>
      </table>	
      <!-- 표가 닫혔습니다 -->
