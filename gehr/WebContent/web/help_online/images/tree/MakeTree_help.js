...
function Folder(folderDescription, hreference, linkChk) { //constructor 
  //constant data 
...
function propagateChangesInState(folder) {   
  ...
  if (folder.isOpen) { 
    ...
    if ( folder.clicked ) {
    	folder.iconImg.src = "../images/tree/ftv2folderopen1.gif" 
    } else {
    ...
    if (folder.nodeImg) {
      if (folder.isLastNode) {
	      if(folder.nChildren == 0) {
	        folder.nodeImg.src = "../images/tree/ftv2lastnode.gif" 
        } else {
		      folder.nodeImg.src = "../images/tree/ftv2plastnode.gif" 
		    }
      } else {
        if(folder.nChildren == 0) {
	        folder.nodeImg.src = "../images/tree/ftv2node.gif" 
    	  } else {
    ...
    if(folder.clicked) {
    	folder.iconImg.src = "../images/tree/ftv2folderclosed1.gif" 
    } else {
...
function initializeFolder(level, lastNode, leftSide){ 
  ...
  if (level>0) {
    if (lastNode) { //the last 'brother' in the children array 
      this.renderOb(leftSide + auxEv + "<img name='nodeIcon" + this.id + "' src='../images/tree/ftv2mlastnode.gif' width=16 height=22 border=0></a>") 
      leftSide = leftSide + "<img src='../images/tree/ftv2blank.gif' width=16 height=22>"  
      this.isLastNode = 1 
    } else { 
      this.renderOb(leftSide + auxEv + "<img name='nodeIcon" + this.id + "' src='../images/tree/ftv2mnode.gif' width=16 height=22 border=0></a>") 
      leftSide = leftSide + "<img src='../images/tree/ftv2vertline.gif' width=16 height=22>" 
      this.isLastNode = 0 
    } 
  } else {
    this.renderOb("") 
...
function drawFolder(leftSide) { 
  ...
  if( this.linkChk == 1) {
    document.write( "<a href=\"" + this.hreference + "\" target='Display'>" )
    document.write("<font color=\"00000\">" + this.desc + "</font>") 
    //style="font-family:����; font-size:9pt; color:#314D5B; background-color:#EFF4F8; border-width:1; bordercolorlight:#DCE7ED; bordercolordark:#FFFFFF; border-style:solid; width:90px;"
    document.write( "</a>" )
  } else {
//    document.write( "<a href=\"#\" target='Display'>" )
...
function clickOnNode(folderId) { 
  ...
  clickedFolder = indexOfEntries[folderId] 
  if (clickedFolder== undefined ) {
     clickedFolder = indexOfEntries[25];
  }
...
function setLink( id , folderId ) {
	var pre = document.formHidden.inputFolder.value
 	if ( pre != -1 ) {
 		...
 		preFolder.iconImg.src = "../images/tree/ftv2folderopen.gif"
 		preFolder.clicked = 0
 	}
    clickedFolder = indexOfEntries[folderId]
    clickedFolder.iconImg.src = "../images/tree/ftv2folderopen1.gif"
...
function gFld(description, hreference, linkChk) { 
  folder = new Folder(description, hreference, linkChk) 
  return folder 
} 
