...
//라디오 선택시 실행.
function changeAppData(PERNR, ENAME){

    if(top.opener.doMethod("setPerson", "X", PERNR, ENAME) == "X") {
...
function PageMove() {
    frm = document.form1;
    frm.action = "<%=WebUtil.JspURL%>"+"common/ApprovalOrganListRight.jsp";
    frm.submit();
}
...
%>
            <tr class="<%=tr_class%>">
              <td><input type="radio" name="radiobutton" value="radiobutton" onClick="javascript:changeAppData('<%=persData.PERNR%>','<%=persData.ENAME%>','<%=persData.ENAME%>','<%=persData.JIKWT%>','<%=persData.JIKKT%>');"></td>
