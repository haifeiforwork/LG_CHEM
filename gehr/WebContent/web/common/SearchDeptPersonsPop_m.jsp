    ...
    if( jobid != null && paging == null ) {
        ...
        try{
            if( jobid.equals("pernr") ) {
                DeptPersInfoData_vt = ( new DeptPersInfoRFC() ).getPersons(i_dept, i_value1, "",       "1", retir_chk, I_IMWON);
            } else if( jobid.equals("ename") ) {
                DeptPersInfoData_vt = ( new DeptPersInfoRFC() ).getPersons(i_dept, "",       i_value1, "2", retir_chk, I_IMWON);
            } else {
                i_value1 = "";
            }
...
<SCRIPT LANGUAGE="JavaScript">
    ...
    function init(){
        ...
        %>
        changeAppData("<%= data.PERNR %>", "<%= data.STAT2%>", "<%= data.ENAME%>" , "<%= data.ORGTX%>", "<%= data.JIKWT%>", "<%= data.JIKKT%>");
        <%
                }
            }
        %>
    ...
    function changeAppData(PERNR, i_stat2, eName, oRgtx, titel, titl2){

        document.form1.empNo.value   = PERNR;
        <%
        //@웹보안 진단 marco257
        //i_stat2 :3 재직자 , 0 이면 퇴직자 I_RETIR
        ...
        document.form1.I_RETIR.value = I_RETIR;
        ...
        opener.$("#n_e_orgtx").val(oRgtx); // 부서
        opener.$("#n_e_titel").val(titel); // 직위
        opener.$("#n_e_titl2").val(titl2); // 직책
        opener.$("#n_ename").val(eName); // 이름
        opener.$("#n_empNo").val("("+PERNR+")"); // 사번
        opener.$("#view_ename").text(eName+ "("+PERNR+")"); // 사번
                    ...
                    <thead><!-- @rdcamel 20161220 -->
                        <tr>
                            ...
                            <th width="*"><spring:message code="LABEL.COMMON.0007"/><!-- 부서 --></th>
              				...
              				<th width="70"><spring:message code='MSG.A.A01.0084'/><!-- 직위/직급호칭 --></th>
                        ...
                        %>
                        <tr class="<%=tr_class %>">
                            <td><input type="radio" name="radiobutton" value="radiobutton" onClick="javascript:changeAppData('<%=deptPersInfoData.PERNR%>', '<%=deptPersInfoData.STAT2%>', '<%=deptPersInfoData.ENAME%>', '<%=deptPersInfoData.ORGTX%>', '<%=deptPersInfoData.JIKWT%>', '<%=deptPersInfoData.JIKKT%>');"></td>
