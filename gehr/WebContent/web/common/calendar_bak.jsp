...
<style type="text/css">
body {background:url(../images/ehr_common/cal_body_bg1.jpg ) no-repeat;}
.calTitle {text-align:center;padding:7px 0 3px 0;margin:0;}
.calDateT {margin-bottom:10px;}
.todayDate {background:url(../images/ehr_common/cal_date_bg_on.gif) no-repeat;}
.todayDate a {color:#fff;text-decoration:none;}
.calDate {background:url(../images/ehr_common/cal_date_bg.gif) repeat-x;}
.calTitleRow {background:url(../images/ehr_common/cal_title_row.gif) no-repeat top center;height:30px;}
.calWrap {border:solid 1px #ddd;width:280px;margin:0 0 0 11px;border-top:solid 2px #eb446b;}
.calTable td {font-weight:bold;}
.calTable td.calDate a {font-familly:verdana;text-decoration:none;color:#838383;}
.calTable td.todayDate a {font-familly:verdana;text-decoration:none;color:#fff}
.calTitle td img {vertical-align:middle;}
.dateT {font-size:14px;color:#757575;font-weight:bold;font-family:malgun gothic}
.dateT select {font-size:12px;color:#333;font-weight:bold;font-family:malgun gothic}
</style>
...
<form name="calen"  method="post">
	...
	<h2 class="calTitle"><img src='<%=WebUtil.ImageURL%>ehr_common/calendar_title.gif' border=0 ></h2>
		<table width="300" border="0" class="calDateT" style="margin:0 0 5px 0;">
        <tr>
          <td width="28%" align=right>
            ...
            <img src='<%=WebUtil.ImageURL%>ehr_common/cal_icon_prev1.gif' border=0 ></a>
          </td>
...
%>
          ...
          <td width="28%" align=left>
            ...
            <img src='<%=WebUtil.ImageURL%>ehr_common/cal_icon_next1.gif' border=0 ></a>
          </td>
        </tr>
      </table>
		<div class="calWrap">
      <table class="calTable" border="0" cellspacing="0" cellpadding="0" align=center>
      	<col width="40" />	<col width="40" />	<col width="40" />	<col width="40" />	<col width="40" />	<col width="40" />	<col width="40" />	<col width="40" />
      	<tr>
      		<td colspan="7" class="calTitleRow"></td>
      	</tr>
      	<!--
        <tr bgcolor="#B9B09C">
...
%>
          <td <%= cur_day==inx ? "class=todayDate" : "class=calDate" %> style="height:30px;width:40px;" align=center>
<%
		if (inx >= 1 && inx <= lastday) {
			wdate =cur_yyyymm+fmt.format(inx);
%>
            <a href="JavaScript:submit_date('<%=wdate%>')"><%= cur_day==inx ? "<b>" : "" %>
              <font size="2" <%= i == 0 ? "color=#eb446b" : i == 6 ? " " : " "%>><span style="font-family:verdana;"><%= inx %></span>
              </font></a>
<%		}%>
         </td>
<%	}%>
