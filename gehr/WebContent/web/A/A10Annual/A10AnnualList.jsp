...
function doSubmit() { //연봉계약서 page로 이동한다.
    document.form2.action = '<%= WebUtil.ServletURL %>hris.A.A10Annual.A10AnnualListSV?subView=<%=subView%>';
...
function trans_form() {
    ...
    if (command !="" ||command =="0") {
        ...
        document.form2.action = '<%= WebUtil.ServletURL %>hris.A.A10Annual.A10AnnualListSV';
        document.form2.method = "post";
        document.form2.submit();
    }
    else {
...
<%
    if( A10AnnualData_vt.size() > 0 ) {
        ...
        if (user.companyCode.equals("C100")) {
		    ...
		    if(!(user.e_persk.equals("11") || user.e_persk.equals("12"))){//// [CSR ID:3006173]
%>
...
%>
			<div class="buttonArea">
			      <ul class="btn_mdl">
			        <li><a href="javascript:trans_form();"><span><spring:message code='LABEL.A.A10.0002' /><!-- 연봉계약서 조회 --></span></a></li>
			      </ul>
...
%>
 ...
 <%if(user.e_persk.equals("11") || user.e_persk.equals("12")){//// [CSR ID:3006173]%>
                ...
                <tr>
                    <th></th>
                    <th><spring:message code='LABEL.A.A10.0003' /><!-- 년도 --></th>
                    <th><spring:message code='MSG.A.A01.0006' /><!-- 소속 --></th>
                    <!-- <td class="td03" width="96">직급/년차</td>
                    <td class="td03" width="96">전년평가등급</td> -->
                    <th><spring:message code='LABEL.A.A10.0004' /><!-- 기본연봉 --></tH>
                    <!-- [CSR ID:3275644]  -->
					<%if(user.empNo.equals("00217646")||user.empNo.equals("00223615")||user.empNo.equals("00221313")){ %>
                    ...
                    <%} %>
                    <th class="lastCol"><spring:message code='LABEL.A.A10.0006' /><!-- 고정연봉 --></th>
                    <!--<td class="td03" width="60">인상율</td> -->
                </tr>
                </thead>
<%}else{ %>
                <colgroup>
                ...
                <thead>
                <tr>
                    <th><spring:message code='MSG.A.A03.0020' /><!-- 선택 --></th>
                    <th><spring:message code='LABEL.A.A10.0003' /><!-- 년도 --></th>
                    <th><spring:message code='MSG.A.A05.0005' /><!-- 소속 --></th>
                    <th><spring:message code='MSG.A.A05.0008' /><!-- 직급/년차 --></th>
                    <th><spring:message code='LABEL.A.A10.0007' /><!-- 전년평가등급 --></th>
                    <th><spring:message code='LABEL.A.A10.0004' /><!-- 기본연봉 --></th>
                    <th><spring:message code='LABEL.A.A10.0008' /><!-- 수당계(월) --></th>
                    <th><spring:message code='LABEL.A.A10.0009' /><!-- 연봉총액 --></th>
                    <th class="lastCol"><spring:message code='LABEL.A.A10.0010' /><!-- 인상율 --></th>
                </tr>
                </thead>
<%} %>
...
// [CSR ID:3006173] 연봉계약서 조회 조건, 1. 2007 이후부터 가능, 2. 임원일 경우 연봉계약서 online 관리를 2016년부터 시작함.
            ...
            if (user.companyCode.equals("C100") && Integer.parseInt(data.ZYEAR) > 2006) {
...
<%
            if (user.companyCode.equals("C100")&&data.ZYEAR.equals("2004")) {
%>
...
%>
                  <td><%= (i==pu.formRow()) && (data.ORGTX).equals("") ? user.e_orgtx : data.ORGTX %>&nbsp;</td>

 <%if(!(user.e_persk.equals("11") || user.e_persk.equals("12"))){//// [CSR ID:3006173]%>
                  <td><%= (i==pu.formRow()) && (data.TRFGR).equals("") ? ( user.e_trfgr+"&nbsp;/&nbsp;"+ ((user.e_vglst).equals("") ? "-" : user.e_vglst )) : ( data.TRFGR+"&nbsp;/&nbsp;"+ ((data.VGLST).equals("") ? "-" : data.VGLST) )%>&nbsp;</td>
                  <td>
<%                int beforYear = Integer.parseInt(data.ZYEAR)-1;
     ...
     //             if (  data.ZYEAR.equals(DB_YEAR)&& (Long.parseLong(StartDate) > Long.parseLong(DataUtil.getCurrentDate())  ) ) {
%>
<%                } else { %>
                  <%= data.EVLVL %>
<%                } %>
...
<%if(data.ZYEAR.equals("2018") && user.empNo.equals("00223401")){ //손지웅%>                    
                    <td>160,800,000&nbsp;</td>
<%}else if(data.ZYEAR.equals("2018") && user.empNo.equals("00005486")){ //김종현%>
                   <td>241,200,000&nbsp;</td> 
<%}else{ %>
