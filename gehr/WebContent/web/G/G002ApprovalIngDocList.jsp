<%--@elvariable id="inputData" type="hris.G.G001Approval.ApprovalListKey"--%>
...
/*   System Name  : e-HR                                                        */
/*   1Depth Name  : HR 결재                                                     */
/*   2Depth Name  : 결재 진행중 문서 리스트                                     */
/*   Program Name : 결재 진행중 문서                                            */
/*   Program ID   : G002ApprovalIngDocList.jsp                                  */
/*   Description  : 문서 목록보기                                               */
/*   Note         : 없음                                                        */
...
<tags:layout title="COMMON.MENU.ESS_APP_ING_DOC">
...
<%--@elvariable id="pu" type="com.sns.jdf.util.PageUtil"--%>
    ...
    <div class="tableInquiry">
        <form id="detailForm" name="detailForm" action="${g.servlet}hris.G.G002ApprovalIngDetailSV">
        ...
        <form id="searchForm" name="searchForm" action="${g.servlet}hris.G.G002ApprovalIngDocListSV">
            <%-- 검색 공통 --%>
        ...
        <div class="table">
            <table class="listTable">
                ...
                <tr>
                    ...
                    <th class="lastCol"><spring:message code="LABEL.G.G02.0009" /><!-- 현재결재자 --></th>
                </tr>
                </thead>
                    <%--<%
                                String statText = "";
                ...
                <c:forEach var="row" items="${resultList}" varStatus="status" begin="${pu.from}" end="${pu.to > 0 ? pu.to - 1 : pu.to}">
                    <tr id="row_${status.index}" class="${f:printOddRow(status.index)}" data-type="${row.UPMU_TYPE}" data-pernr="${row.PERNR}" data-seq="${row.AINF_SEQN}"  >
                        ...
                        <td>
                            <c:choose>
                                <c:when test="${row.STAT_TYPE == '1'}"><spring:message code="COMMON.APPROVAL.LIST.ING.1"/></c:when>
                                <c:when test="${row.STAT_TYPE == '2'}"><spring:message code="COMMON.APPROVAL.LIST.ING.2"/></c:when>
                                <c:when test="${row.STAT_TYPE == '3'}"><spring:message code="COMMON.APPROVAL.LIST.ING.3"/></c:when>
                                <c:when test="${row.STAT_TYPE == '4'}"><spring:message code="COMMON.APPROVAL.LIST.ING.4"/></c:when>
                            </c:choose>
                        </td>
                        <td class="lastCol">${row.DETERMINANT}</td>
                    </tr>
                </c:forEach>
