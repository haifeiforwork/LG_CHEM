...
function trans_form() {
    ...
    for (var i = 0; i < opener.document.form1.CONG_CODE.length ; i++) {
       if (opener.document.form1.CONG_CODE[i].value == eval("document.form1.CONG_CODE"+command+".value") ){
           opener.document.form1.CONG_CODE[i].selected = true;
       }
    }
    eval("opener.document.form1.CONG_DATE.value   = addPointAtDate(document.form1.CONG_DATE"+
    		command+".value.replace('-','').replace('-',''))");
    eval("opener.document.form1.HOLI_CONT.value   = parseInt(document.form1.HOLI_CONT"+command+".value)");
    eval("opener.document.form1.P_A024_SEQN.value = document.form1.AINF_SEQN"+command+".value");
    eval("opener.document.form1.REASON.value = document.form1.CONG_NAME"+command+
    		".value +  '/' +document.form1.RELA_NAME"+command+".value + '/'+ addPointAtDate(document.form1.CONG_DATE"+
    		command+".value.replace('-','').replace('-',''))" );
