...
/*   2Depth Name  : 월급여                                                      */
/*   Program Name : 월급여                                                      */
/*   Program ID   : D05MpayDetail_m.jsp                                         */
...
<%
    ...
    WebUserData user = WebUtil.getSessionUser(request);
	    ...
	    if ( user_m != null ) {
	        ...
	        KONGSU_HOUR = (String)kongsuVec.get(0);
	        KONGSU_PAY = (String)kongsuVec.get(1);
	        ...
	        overSeaYn = user_m.e_werks.substring(0,1); //해외급여자여부 :E
	    }
	    else
	        ableyear  = "1800";
	    ...
	    D05CompensationData d05CompensationData = null;
	    if(user_m != null){
	        d05CompensationData = (D05CompensationData)rfc.getDetail(user_m.empNo, begym, endym);
	        //D05CompensationData d05CompensationData = null;//(D05CompensationData)rfc.getDetail(user.empNo, begym, endym);
	    }
	    //-------------------------------------------------------------------------------
...
function  doSearchDetail() {
    ...
    document.form1.action = "<%= WebUtil.ServletURL %>hris.D.D05Mpay.D05MpayDetailSV_m";
...
<%
    if ( user_m != null ) {
%>
    small_window=window.open("<%= WebUtil.JspURL %>D/D05Mpay/D05HealthInsPop_m.jsp?I_PERNR=<%= user_m.empNo %>&I_YEAR=<%= year %>&BET02="+bet02,"D05MpayHI",
...
function doPayment() {
    ...
    document.form3.jobid.value  = "search";
    ...
    document.form3.action = "<%= WebUtil.ServletURL %>hris.D.D06Ypay.D06YpayDetail_to_yearSV_m";
    document.form3.method = "post";
    document.form3.submit();
}
...
function doTotCompensation() {
    ...
    document.form2.action = "<%= WebUtil.JspURL %>D/D05Mpay/D05CompensationDetail.jsp?type=Y";
    document.form2.method = "post";
    document.form2.submit();
}
//-->
...
<% //@v1.3
  ...
  if (user_m != null && "N".equals(O_CHECK_FLAG) ) {
                  ...
                  <table width="780" border="0" cellspacing="0" cellpadding="0">
                      <tr>
                          <td align="center" class="title02">
                              <font size="3">
행복한 가정을 만들고 풍요로운 삶을 살며 자아를 실현시키겠다는<BR/>
여러분의 꿈이 오늘도 LG화학을 움직이고 있습니다.
                              </font>
                          </td>
                      </tr>
                  </table>
...
%>
                    ...
                    <td class="align_right"><a class="inlineBtn" href="javascript:kubya();">
                ...
                <tr>
                    ...
                    <th class="th02"><spring:message code="LABEL.D.D05.0005"/></th><!--사번  -->
                    <td><%= user_m.empNo %></td>
                ...
                <tr>
                    ...
                    <!-- CSRID : 2559989 일당 -> 일급 -->
                    <!-- <td  width="100">일당</td> -->
                    <th ><spring:message code="LABEL.D.D05.0008"/></th><!--  기본급-->
...
%>
    ...
    <div class="align_center">
        <p><spring:message code="MSG.COMMON.0004" /></p>
    </div>
--%>
...
%>
    ...
    <%=d05MpayDetailData4.ZCURR%>1=KRW<%= WebUtil.printNum( Double.parseDouble(d05MpayDetailData4.ZRATE),"###,###.00")%>/<%=d05MpayDetailData4.ZCURR1%>1=KRW<%= WebUtil.printNum( Double.parseDouble(d05MpayDetailData4.ZRATE1),"###,###.00")%> )</p>
  ...
  <div class="listArea">
      <div class="table">
          ...
          <th><spring:message code="LABEL.D.D05.0015" /></th><!-- 금액 -->
          <th class=""><spring:message code="LABEL.D.D05.0016" /></th><!-- 공제내역 -->
    ...
    %>
                <a href="javascript:doOverSeaETC();"><b><%= data5.LGTXT.equals("") ? "　" : data5.LGTXT %></b></a> <font class="tds2"><spring:message code="LABEL.D.D05.0024" /><!--註1)--></font><br>
    ...
    %>
                      ...
                      <td class=" divide align_left" style="vertical-align: top;">
    <%
   ...
   <!--[CSR : 2591949] 각주 2번 (국민연금, 고용보험) 삭제 및 소득세, 주민세를 각주 2로 변경  -->
    ...
    %>
                    ...
                    <tr class="sumRow">
                      ...
                      <td class="align_right"><%= WebUtil.printNumFormat(d05MpayDetailData5.BET01) %><br>
                      (<%=d05MpayDetailData4.ZCURR%> <%= WebUtil.printNum(bet01,"###,###.00") %>)</b></td>
                      <td class=""><spring:message code="LABEL.D.D05.0023" /></td><!-- 공제계 -->
                      <td class="lastCol align_right" align="right"><%= WebUtil.printNumFormat(d05MpayDetailData5.BET02) %><br>
    ...
    <table>
      ...
      <td width="380">
        ...
        <span style="padding:8px"></span><spring:message code="LABEL.D.D05.0031" /><br></font>    <!-- 한시적 면제 -->
<%
        ...
        <div class="table">
            ...
            <thead>
                <tr>
          		...
          		<th class=""><spring:message code="LABEL.D.D05.0015" /></th>   	<!-- 금액 -->
...
<%
        for( int i = 0 ; i < d05MpayDetailData5_vt.size() ; i++ ) {
            ...
            if( !data5.LGTXT.equals("소급분총액") ) {
%>
<% // 성과급 중, 30만원 해당임금에 대해 생산성향상급으로 표시(임시). 2004.7.20. mkbae.
...
<%
                ...
                if(  !data5.BET01.equals("0") && data5.LGTXT.equals("소급분총액") ) {  //k_yn.equals("Y") && 제거.
%>
                          &nbsp;<br> <br> <a href="javascript:doSogub();"><font color="#CC3300" weight="900">
...
<%
        ...
        for ( int i = 0 ; i < d05MpayDetailData5_vt.size() ; i++ ) {
            ...
            if(data5.LGTXT.equals("소급분총액")) {
%>
                          &nbsp;<br> <br> <%= data5.BET01.equals("0") ? "　" : WebUtil.printNumFormat(data5.BET01) %>&nbsp;<br>
<%
            }
        }
%>
...
%>
                          <%= data2.LGTX1 %><br>
<%
//             } else if(  !data2.ANZHL.equals("0") && data2.LGTX1.equals("소급분공제총액") ) {
            } else if(  data2.LGTX1.equals("소급분공제총액") ) {
%>
                          <a href="javascript:doSogub();"><font color="#CC3300" weight="900"><%= data2.LGTX1.equals("") ? "　" :  data2.LGTX1%></font></a>
<%
...
<%}//[CSR ID:3589279] 임시적용 %>
                        <td class=" align_right"><%= WebUtil.printNumFormat(d05MpayDetailData5.BET01) %>&nbsp;</td>
...
<%
	if(Integer.parseInt(yymm) >= 201807 ){
        if( ocrsn_t.equals("01") || ocrsn_t.equals("ZZ") ) {//월급여 + TOTAL
            if(user_m.e_persk.equals("21") || user_m.e_persk.equals("22") || (user_m.e_persk.equals("24") && !user_m.e_jikkb.equals("AC0")) ){//계약직 중, 사외이사는 제외
%>            	
  <div class="commentsMoreThan2">
...
<%
                for ( int i = 0 ; i < d05MpayDetailData4_vt.size() ; i++ ) {
                    ...
                    //[CSR ID:3391262]
                    //[CSR ID:3493623]
                    //if ( kase_text.equals("과세반영") ||  kase_text.equals("G6 상")) {
                    if ( kase_text.equals("과세반영") ||  (!kase_text2.equals("소급분") && data4.LGTX1.indexOf("과세반영")>-1)) {
%>
...
%>
    ...
    <h2 class="subtitle"><spring:message code="LABEL.D.D05.0081" /></h2>	<!-- 근태현황 -->
        ...
        <div class="table">
            <table class="tableGeneral">
                <tr>
                    ...
                    <td class="align_right" colspan=3><a href="javascript:doTotCompensation();"><font color="#CC3300" weight="900">
                         <%= WebUtil.printNumFormat((ytot+Double.parseDouble(d05CompensationData.BET02))*100,0) %>&nbsp;</font></a></td>
                </tr>
                <tr>
            ...
            <span class="commentOne"><spring:message code="LABEL.D.D05.0090" />
            	<!-- 상기 <B>복리후생 총액</B>은, 법정 복리후생을 포함하여 법정 外 복리후생 항목 中 개인 別로 지원된 연간 누계액을 표기한 것입니다. 자세한 내역은 해당항목을 클릭하여 조회 바랍니다. -->
            </span>
        </div>
    </div>
...
<%
    ...
    // [CSR ID:2583929] 생산기술직 38 추가
    if( user_m != null && !"N".equals(O_CHECK_FLAG) && (user_m.e_persk.equals("31") ||user_m.e_persk.equals("32")||user_m.e_persk.equals("33") || user_m.e_persk.equals("38") )) {
%>
        <div><spring:message code="LABEL.D.D15.0119" />
        <!--
        근태, 월급여, 연급여 및 성과급 등 개인 처우 관련 사항을 社內外 제3자에게 절대로 공개하지 마시기 바라며,<br>
        이를 위반 시에는 취업규칙상의 규정과 절차에 따라 징계조치 됨을 알려 드립니다.
         -->
        </div>
<%
    }
...
<%
    ...
    if( user_m != null && !"N".equals(O_CHECK_FLAG) && (user_m.e_trfar.equals("02") || user_m.e_trfar.equals("03") || user_m.e_trfar.equals("04"))  ) {
%>
...
%>
 ...
 <!--  [CSR ID:2995203]@v1.4 보상명세서 항목추가 -->
  ...
  <form name="form4" method="post" action="">
    ...
    <input type="hidden" name="pernr" value="<%= user_m.empNo %>">
    <input type="hidden" name="ename" value="<%= user_m.ename %>">
<%
