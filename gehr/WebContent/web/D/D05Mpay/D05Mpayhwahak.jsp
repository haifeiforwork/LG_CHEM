...
<div class="payWrap">
	...
	<div class="contentWrap">
     ...
     <!-- [CSR ID:2781737] 급여명세표 문구 삭제 급여 문구 부분 삭제 -->
		...
		<h2 class="subtitle" style="margin-bottom:0"><%= year %><spring:message code="LABEL.D.D15.0020"/>  
		<%= month %><spring:message code="LABEL.D.D15.0021"/></h2>
      	...
      	<div class="tableArea">
      		<div class="table" >
	            ...
	            <colgroup>
	              ...
	              <tr >
	                <th width="70" ><spring:message code="LABEL.D.D05.0004"/><!-- 부서명 --></th>
	                <td><%= user.e_orgtx %></td>
	                <th width="70" class="th02"><spring:message code="LABEL.D.D05.0005"/><!-- 사번 --></th>
	                <td><%= user.empNo %></td>
	                <th width="70" class="th02"><spring:message code="LABEL.D.D05.0006"/><!-- 성명 --></th>
	                <td><%= user.ename %></td>
	              </tr>
...
<!--  [CSR ID:2575929] 기준환율 USD 추가 -->
	...
	%>
	                    <a href="javascript:doSogub();"><font class="font03" weight="900"><%= data2.LGTX1.equals("") ? "　" :  data2.LGTX1%></font></a>
	<%
	...
	%>
	                ...
	                <tr class="sumRow">
	                  ...
	                  <td class="align_right divider"><%= WebUtil.printNumFormat(d05MpayDetailData5.BET01) %>&nbsp;<br>
	                  	(<%=d05MpayDetailData4.ZCURR%> <%= WebUtil.printNum(bet01,"###,###.00") %>)&nbsp;</b>
	                  </td>
	                  <td><spring:message code="LABEL.D.D05.0023" /></td><!-- 공제계 -->
	                  <td class="lastCol align_right"><%= WebUtil.printNumFormat(d05MpayDetailData5.BET02) %>&nbsp;<br>
...
<%} %>
	...
	<!-- 	 [CSR ID:2591949] 2번 각주 삭제 및 3번 각주가 2번 각주로 수정. 	          註2)</font> 국민연금/고용보험 : 연중 2회 합산 납부(6, 12월)<br> -->
			<div><spring:message code="LABEL.D.D05.0027" /> <spring:message code="LABEL.D.D05.0028" />
    			<!--註2)해외발생 개인 소득세는 회사에서 전액 보전. 단, 국내 연봉을 기준으로 소득세/주민세를 산정하여 매월 원천징수.<br>
...
%>
			<div><spring:message code="LABEL.D.D05.0122" /><!-- ※ 국민연금/고용보험 : 당사에서 공단에 선납부후, 연중 2회(상반기,하반기) 개인에게 송금요청 함-->
			</div>
<%} %>
...
<%  }  %>
                ...
                <td class="divide align_right" height="120" style="vertical-align: top;">
<%
    for ( int i = 0 ; i < d05MpayDetailData5_vt.size() ; i++ ) {
    D05MpayDetailData2 data5 = (D05MpayDetailData2)d05MpayDetailData5_vt.get(i);
%>
...
<%}//[CSR ID:3589279] 임시적용 %>                  
                <td class="divide align_right"><%= d05MpayDetailData5.BET01.equals("0") ? "" : WebUtil.printNumFormat(d05MpayDetailData5.BET01) %>&nbsp;</td>
                <td>공제계</td>
                <td class="lastCol align_right"><%= d05MpayDetailData5.BET02.equals("0") ? "" : WebUtil.printNumFormat(d05MpayDetailData5.BET02) %>&nbsp;</td>
              </tr>
              </thead>
            </table>
        </div>
    ...
    <td><table width="600" border="0" cellspacing="0" cellpadding="2">
      ...
      <table width="600" border="0" cellspacing="1" cellpadding="2" class="table02">
        ...
        <tr>
          ...
          <td  width="200" align="right"><%= WebUtil.printNumFormat(Double.parseDouble(d05CompensationData.BET02)*100,0) %>&nbsp;</tr>
        </tr>
      </table>
    </td>
  </tr>
...
<%}//38생산기술직만 추가되게...
                                    ...
                                    <table width="600" border="0" cellspacing="1" cellpadding="2" class="table02" height="100">
                                        <tr>
                                            <td  width="100"><spring:message code="LABEL.D.D05.0091" /><!-- 공지사항 --></td>
                                            <td  style="vertical-align: top; padding-top:15px">
