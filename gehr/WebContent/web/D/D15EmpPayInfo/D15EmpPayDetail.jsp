...
<tags:layout css="ui_library_approval.css" script="dialog.js" >
    ...
    <tags-approval:detail-layout titlePrefix="LABEL.D.D15.0201" updateUrl="${g.servlet}hris.D.D15EmpPayInfo.D15EmpPayChangeSV" disableUpdate="true" >
            ...
            <div class="table">
                <table class="listTable">
                    ...
                    <tr>
                        ...
                        <th class="lastCol" ><spring:message code="LABEL.D.D15.0203" /><!-- 통화 --></th>
                    </tr>
                    </thead>
                    ...
                    <c:forEach  var="row" items="${resultList}" varStatus="status">
                        ...
                        <tr id="-pay-row-${status.index}" class="${f:printOddRow(status.index)}">
                            <td>${row.PERNR}</td>
                            <td>${row.ENAME}</td>
                            <td class="align_left" style="padding-left: 10px;">[${row.LGART}] ${row.LGTXT}</td>
                            <td class="align_right">${f:convertCurrencyDecimal(row.BETRG, '') }</td>
                            <td class="lastCol">${row.WAERS}</td>
                        </tr>
                    </c:forEach>
                    <tags:table-row-nodata list="${resultList}" col="5"/>
                    <script>
