...
/*   Program ID   : D25WorkTimeLeaderDailyExcel.jsp
/*   Description  : 일별 근무 입력 현황 Excel
/*   Note         : 
...
%><%@ include file="/web/D/D25WorkTime/D25WorkTimeCommonPreprocess.jsp" %><%
    ...
    String attachmentName = g.getMessage("LABEL.D.D25.N1009") + " - " + g.getMessage("TAB.COMMON.0123"); // 근무 입력 현황 - 일별
...
<table>
    ...
    <thead>
        ...
        <tr>
            ...
            <th class="worktime-sum"><spring:message code="LABEL.D.D25.N2116" /></th><%-- 계 --%><c:if test="${!empty TABLES and !empty TABLES.T_TLDAYS}"><c:forEach items="${TABLES.T_TLDAYS}" var="dateData">
            <th class="${dateData.HOLID eq 'X' ? 'holiday' : (f:getDay(fn:replace(dateData.WKDAT, '-', '')) % 6 eq 1 ? 'weekend' : 'weekday')}">${dateData.WDAYS}<br />(${dateData.WOTAG})</th></c:forEach></c:if>
        </tr>
    </thead>
    <tbody>
<c:choose><c:when test="${empty TABLES or empty TABLES.T_ORGDAYS}">
        <tr>
            <td></td>
            <td colspan="${fn:length(TABLES.T_TLDAYS) + 4}"><spring:message code="MSG.COMMON.0004"/></td><%-- 해당하는 데이타가 존재하지 않습니다. --%>
        </tr>
</c:when><c:otherwise><c:forEach items="${TABLES.T_ORGDAYS}" var="rowData">
        <tr>
            <th></th>
            <td>${ rowData.ENAME }</td><%-- 이름 --%>
            <td>${ rowData.ORGTX }</td><%-- 소속 --%>
            <td>${ rowData.JIKKT }</td><%-- 직책 --%>
            <td class="worktime-sum">${ rowData.CTOTDW }</td><%-- 계 --%><c:forEach items="${TABLES.T_TLDAYS}" var="dateData">
            <td class="${dateData.HOLID eq 'X' ? 'holiday' : (f:getDay(fn:replace(dateData.WKDAT, '-', '')) % 6 eq 1 ? 'weekend' : 'weekday')}">${ rowData[fn:replace('CDWS#', '#', dateData.WDAYS)] }</td></c:forEach>
        </tr>
