...
/*   Program ID   : D02ConductListDe.jsp                                          */
...
function initTable(){
    ...
    /*
      for (var i=0;i<tab_h.rows[2].cells.length;i++){
         tab_c.rows[0].cells[i].width = tab_h.rows[2].cells[i].offsetWidth-1;
      }
    */
    <%}%>
}
...
%>
    ...
    <% if(!E_RETURN.trim().equals("S")) { %>
                    ...
                    <td colspan="10"><!--No data --><%=g.getMessage("LABEL.D.D04.0039")%></td>
...
%>
     ...
     <tr class="borderRow" style="height: 15px;">
            <td <%= isWeekend ? "class=\"sunday\"" : "class=\"\""%> width="77"><%=WebUtil.printDate(detailData.DATE) %></td>
