...
function goDetail( i ) {
    arguSetting( i ) ;
    document.form2.action = '<%= WebUtil.ServletURL %>hris.D.D09Bond.D09BondDetailSV_m' ;
...
function goDpotTota( value ) {
    ...
    document.form2.action = '<%= WebUtil.ServletURL %>hris.D.D09Bond.D09BondDepositSV_m' ;
    document.form2.method = 'post' ;
    document.form2.submit() ;
}
...
<jsp:include page="/include/body-header.jsp">
    <jsp:param name="title" value="COMMON.MENU.MSS_PY_GANI_AMOU"/>
...
<%
    if( D09BondListData_vt.size() > 0 ) {
        for ( int i = 0 ; i < D09BondListData_vt.size() ; i++ ) {
            ...
            D09BondCortData_vt = func1.getBondCort( user_m.empNo, data.CRED_NUMB, data.SEQN_NUMB );
%>
          <tr class="<%=tr_class%>">
            <td><%= i + 1 %></td>
...
%>
            ...
            <%= data.ENDDA.equals( "0000-00-00" ) ? "" : WebUtil.printDate( data.ENDDA ) %></td>
          </tr>
...
%>
        ...
        <div class="table">
            <table class="tableGeneral">
              <tr>
                ...
                <td class="align_right"><%= WebUtil.printNumFormat( G_SUM ) %> <!-- 원 --><%=g.getMessage("LABEL.D.D15.0010")%></td>
                <th class="th02"><!-- 미공제 잔액 --><%=g.getMessage("LABEL.D.D15.0032")%></th>
                <td class="align_right"><%= WebUtil.printNumFormat( dedu_rest ) %> <!-- 원 --><%=g.getMessage("LABEL.D.D15.0010")%></td>
              </tr>
              <tr>
                ...
                <td class="align_right"><%= WebUtil.printNumFormat( coll_rest ).equals("-0") ? "" : WebUtil.printNumFormat( coll_rest ) %> <!-- 원 --><%=g.getMessage("LABEL.D.D15.0010")%></td>
              </tr>
            </table>
        </div>
    </div>
    ...
    <div class="buttonArea">
        <ul class="btn_crud">
            <li><a href="<%= WebUtil.ServletURL %>hris.D.D09Bond.D09BondDeductionSV_m"><span><!-- 공제 현황 조회 --><%=g.getMessage("LABEL.D.D15.0035")%></span></a></li>
        </ul>
    </div>
<%
    } else {
%>
          <tr class="oddRow">
            <td class="lastCol" colspan="8"><!-- 해당하는 데이터가 존재하지 않습니다. --><%=g.getMessage("MSG.D.D15.0013")%></td>
          </tr>
        </table>
      </div>
    </div>
