...
/*   2Depth Name  : 연급여                                                      */
/*   Program Name : 연급여                                                      */
/*   Program ID   : D06YpayPrint_to_year_m.jsp                                 */
...
function doPrint() {
    ...
    window.open('', 'essPrintWindow', "toolbar=0,location=0,directories=0,status=0,menubar=0,resizable=0,width=900,height=660");
     ...
     <div class="tableArea">
          <div class="table">
            <table class="tableGeneral">
        		<thead>
                <tr>
                  <th  width="100"><%=g.getMessage("LABEL.D.D06.0003")%></td> <!-- 조회년도 -->
                  <td colspan="7" class="align_left"> <select name="from_year">
...
%>
                ...
                <tr>
                  <th width="100"><%=g.getMessage("LABEL.D.D05.0004")%></td><!-- 부서명 -->
                  <td  class="align_left" colspan="3"><%= user_m.e_orgtx %></td>
                  <th width="100" class="th02"><%=g.getMessage("LABEL.D.D05.0005")%></td><!-- 사번 -->
                  <td class="align_left"  width="100" ><%= user_m.empNo %></td>
                  <th width="100" class="th02"><%=g.getMessage("LABEL.D.D06.0006")%></td><!-- 성명 -->
                  <td class="align_left"  width="100" ><%= user_m.ename %></td>
                </tr>
              </table>
              <!--조회기간 테이블 끝-->
            </td>
          ...
          <tr>
            <td>
              <!-- 상단 검색테이블 시작-->
              <table width="780" border="0" cellspacing="1" cellpadding="1" class="table02">
                <tr>
                  <td ><br><spring:message code="MSG.COMMON.0004" /><br><br></td><!-- 해당하는 데이타가 없습니다. -->
                </tr>
               </table>
             </td>
          </tr>
   ...
   <div class="listArea">
      <div class="table">
        <table class="mpayTable">
        		<thead>
                <tr>
                  <th  rowspan="2" width="50" valign="center">
                  ...
                  <th  class="lastCol" rowspan="2" width="60" valign="center">
                        <spring:message code="LABEL.D.D05.0012" /></td><!-- 차감지급액 -->
                </tr>
                <tr>
                  <th  width="8%"><spring:message code="LABEL.D.D06.0022" /></th><!-- 급여 -->
                  <th  width="8%"><spring:message code="LABEL.D.D06.0023" /></th><!-- 상여 -->
                  <th  width="8%"><spring:message code="LABEL.D.D05.0022" /></th><!-- 지급계 -->
                  <!-- [CSR ID:2995203] 명칭변경<th  width="70" >갑근세</th> -->
                  ...
                  <th  width="8%"><spring:message code="LABEL.D.D06.0005" /></th><!-- 기타 -->
                  <th  width="8%"><spring:message code="LABEL.D.D05.0023" /></th><!--  공제계-->
                </tr>
                <%
...
%>
                <tr>
                  ...
                  <td ><%= kita1.equals("0.0") ? "" : WebUtil.printNumFormat(kita1) %>&nbsp;</td>
...
%>
                <%-- <tr class="sumRow">
                  ...
                  <td ><%= kita_totl1.equals("0.0") ? "" : WebUtil.printNumFormat(kita_totl1) %>&nbsp;</td>
                  <td ><%= total14.equals("0") ? "" : WebUtil.printNumFormat(total14) %>&nbsp;</td>
                  <td class="lastCol"><%= chgam_total1.equals("0") ? "" : WebUtil.printNumFormat(chgam_total1) %>&nbsp;</td>
                </tr> --%>
                <%
       // } else {
            String total12      = (String)request.getAttribute("total12");
    ...
    <div class="listArea">
      <div class="table">
        <table class="mpayTable">
        		<thead>
                <tr>
                  <th  rowspan="2" width="8%" valign="center">
                        <spring:message code="LABEL.D.D05.0002" /></th><!-- 해당년월 -->
										                  <!-- <th  rowspan="2" width="60">생산직<br>비과세</th>
										                  <th  rowspan="2" width="55">
										                        <spring:message code="LABEL.D.D06.0008" /></th> 노조비 [CSR ID:2995203] -->
                  <th  rowspan="2" width="10%">
                        <spring:message code="LABEL.D.D06.0013" /></th><!--  주택자금<br>이자지원-->
                  <th  colspan="8" valign="center">
                        <spring:message code="LABEL.D.D06.0014" /></th><!-- 과세반영 -->
                  <th  rowspan="2" width="10%" class="lastCol">
                        <spring:message code="LABEL.D.D15.0070" /></th><!-- 계 -->
                </tr>
                <tr>
                  ...
                  <th  width="9%"><spring:message code="LABEL.D.D06.0019" /></th><!-- 장기<br>근속상 -->
...
%>
    ...
    <!--    [CSR ID:2995203]               <td > <%
            for ( int j = 0 ; j < D06YpayDetailData3_vt.size() ; j++ ) {
                ...
                if( yymm.equals(data9.YYMMDD)&& data9.LGTX4 != null &&data9.LGTX4.equals("생산직비과세")&& data9.BET04 != null  ) {
                    if(d_yymm.equals(data9.YYMMDD)) {
                        continue;
                    }else{
                        d_yymm = data9.YYMMDD;
%> <%= data9.BET04.equals("0") ? "　" : WebUtil.printNumFormat(data9.BET04) %> <%
                        //sum2 += Double.parseDouble(data9.BET04);
                        //sum12 += Double.parseDouble(data9.BET04);
                    }
                }
            }
%> &nbsp;</td> -->
...
%>
                <tr class="sumRow">
                  ...
                  <td ><%= WebUtil.printNumFormat(sum_san).equals("0") ? " " : WebUtil.printNumFormat(sum_san) %>&nbsp;</td><!--@v1.1산학자금추가-->
...
<%  }     %>
                  ...
                  <!-- [CSR ID:2995203]<font color="#006699">※ 근로소득원천징수영수증의
                    총급여액은 연간 급여/상여 합계금액과 인정이자를 합산한 금액임.</font>-->
