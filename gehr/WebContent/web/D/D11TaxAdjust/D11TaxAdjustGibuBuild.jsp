...
/*   2Depth Name  : 연말정산                                                    */
/*   Program Name : 연말정산                                                    */
/*   Program ID   : D11TaxAdjustGibuBuild.jsp                                   */
...
/*                    2012-12-13  C20121213_34842 기부금유형: 당해년도 입사자만 기부금 유형 신탁, 특례기부금 조회됨 */
...
//  2002.12.03 내역조회가 가능한지 날짜를 체크한다. 
    ...
    //@2011  전근무지 메뉴를 해당년도 입사자인 경우에만 메뉴를 보여준다.
    String Prev_YN="";
...
function check_data() {
    for( var i = 0 ; i < "<%= gibu_count %>" ; i++ ) {
        ...
        if ( gubun == "1" || gubun == "9") return true;  //자동반영분은 체크하지 않음
...
function do_build() {
    ...
    if ( check_data() ) {
        ...
        for( var i = 0 ; i < "<%= gibu_count %>" ; i++ ) {
            ...
            eval("document.form1.DONA_YYMM"+i+".value  = removeResBar(document.form1.DONA_YYMM"+i+".value);");
            ...
            if (  eval("document.form1.DONA_AMNT"+i+".value.length") > 9 ) {
                alert("<spring:message code='MSG.D.D11.0050' />"); //신청금액을 확인하세요!
                eval("document.form1.DONA_AMNT"+i+".focus()")
                return;
            }
...
function lastDay(i,date)
{
    ...
    month = date.value.substring(4,6);
    if ( date.value == "" ) {
        return true;
    }
...
//@v1.1 사업자번호 명칭검색 //@2014 연말정산 박난이S 요청 해당 function 호출 안함.
...
%>
    ...
    %>
             ...
             <!--CSR ID:1361257 관계,성명,주민번호 추가 -->
            <td>
              <select name="SUBTY<%=i%>"  style="width=70px" auto disabled>
                <option value="">-------------</option>
<%= WebUtil.printOption((new D11FamilyRelationRFC()).getFamilyRelation(""), data.SUBTY) %>
            ...
            <td>
              ...
              <select name="F_ENAME<%=i%>" style="width=60px" auto onChange="javascript:subty_change(<%=i%>,this);">
                <option value="">--------</option>
<%
    old_Name = "";
    for( int j = 0 ; j < MediPeP_vt.size() ; j++ ){
...
%>
            ...
            <td>
              <input type="text" name="F_REGNO<%=i%>" value="<%= data.F_REGNO.equals("") ? "" : DataUtil.addSeparate(data.F_REGNO) %>" size="13" readonly maxlength="14" onBlur="javascript:resno_chk(this);" >
            </td>
            ...
            <!--CSR ID:1361257 관계,성명,주민번호 추가 -->
                ...
                <td>
                  ...
                  <select name="DONA_CODE<%= i %>" style="width=90px" auto onChange="javascript:dona_numb_check(<%= i %>,this);">
                    <option value="">-------------</option>
    <%= WebUtil.printOption((new D11TaxAdjustDonationTypeRFC()).getDonationType(targetYear,PERNR), data.DONA_CODE) %>
                  </select>
                </td>
                <td>
                  <input type="text" name="DONA_YYMM<%= i %>" value="<%= data.DONA_YYMM.equals("") ? "" : data.DONA_YYMM %>" size="10" maxlength="6" style="text-align:center" onBlur="javascript:prev_check(this,<%=i%>);if (!lastDay(<%=i%>,this)) return false;">
                </td>
                <td>
                  <input type="text" name="DONA_DESC<%= i %>" value="<%= data.DONA_DESC.equals("") ? "" : data.DONA_DESC %>" size="15" maxlength="20" >
                </td>

                <td>
                  <input type="text" name="DONA_NUMB<%= i %>" value="<%= data.DONA_NUMB%>" size="13" maxlength="10" style="text-align:center" onBlur="javascript:name_search(this,<%=i%>);businoFormat1(<%=i%>,this);resno_chk(this);"><!-- name_search(this,< %=i%>);박난이S 요청 -->
                </td>
                <td>
                  <input type="text" name="DONA_COMP<%= i %>" value="<%= data.DONA_COMP.equals("") ? "" : data.DONA_COMP %>" size="15" maxlength="20" onkeypress="javascript:fn_checkSpText(this);">
                </td>

                <td>
                  <input type="text" name="DONA_AMNT<%= i %>" value="<%= data.DONA_AMNT.equals("") ? "" : WebUtil.printNumFormat(data.DONA_AMNT) %>" size="10" maxlength="11" onKeyUp="javascript:moneyChkEventForWon(this);" style="text-align:right" >
                </td>
                  <input type="hidden" name="CHNTS<%=i%>" value="<%=  data.CHNTS.equals("") ? "" : data.CHNTS %>" <%= data.CHNTS.equals("X")  ? "checked" : "" %>>
             <% if ( Prev_YN.equals("Y") ){ //@2011 %>
                <td>
                  <input type="checkbox" name="DONA_CRVIN<%= i %>" value="<%=  data.DONA_CRVIN.equals("") ? "" : data.DONA_CRVIN %>" <%= data.DONA_CRVIN.equals("X")  ? "checked" : "" %>  class="<%= data.GUBUN.equals("1") ? "input04" : "input03" %>" style="text-align:center"  onClick="javascript:prev_check(this,<%=i%>);dona_crvin_check(<%= i %>,this);">
                </td><!--이월공제-->
                <td>
                  <input type="text" name="DONA_CRVYR<%= i %>" value="<%= data.DONA_CRVYR.equals("0000") ? "" : data.DONA_CRVYR %>" size="4" maxlength="4" style="text-align:center"    disabled >
                </td><!--이월공제년도-->
                <td>
                  <input type="text" name="DONA_DEDPR<%= i %>" value="<%= data.DONA_DEDPR.equals("") ? "" : WebUtil.printNumFormat(data.DONA_DEDPR) %>" size="10" maxlength="11"  class="<%= data.GUBUN.equals("1")||data.DONA_CRVIN.equals("") ? "input04" : "input03" %>" onKeyUp="javascript:moneyChkEventForWon(this);" style="text-align:right"   disabled >
                </td><!--전년까지공제액-->
             <% }else{ %>
...
%>
            ...
            <td>
              <input type="text" name="F_REGNO<%=i%>" value="" size="13" readonly maxlength="14" onBlur="javascript:resno_chk(this);" >
            </td>
    ...
    <%= WebUtil.printOption((new D11TaxAdjustDonationTypeRFC()).getDonationType(targetYear,PERNR), "") %>
                ...
                <td>
                  <input type="text" name="DONA_YYMM<%= i %>" value="" size="10" maxlength="6" style="text-align:center" onBlur="javascript:prev_check(this,<%=i%>);if (!lastDay(<%=i%> ,this)) return;">
                </td>
                <td>
                  <input type="text" name="DONA_DESC<%= i %>" value="" size="15" maxlength="40">
                </td>
                <td>
                  <input type="text" name="DONA_NUMB<%= i %>" value="" size="13" maxlength="15" style="text-align:center" onBlur="javascript:name_search(this,<%=i%>);businoFormat1(<%=i%>,this);resno_chk(this);"><!-- name_search(this,< %=i%>);박난이S 요청 -->
                </td>
                <td>
                  <input type="text" name="DONA_COMP<%= i %>" value="" size="15" maxlength="20" onkeypress="javascript:fn_checkSpText(this);">
                </td>

                <td>
                  <input type="text" name="DONA_AMNT<%= i %>" value="" size="10" maxlength="11" onKeyUp="javascript:moneyChkEventForWon(this);" style="text-align:right" >
                </td>
                  <input type="hidden" name="CHNTS<%=i%>" value="">
             <% if ( Prev_YN.equals("Y") ){ %>
                <td>
                  <input type="checkbox" name="DONA_CRVIN<%= i %>" value="" style="text-align:center" onClick="javascript:dona_crvin_check(<%= i %>,this);prev_check(this,<%=i%>);">
                </td><!--이월공제-->
                <td>
                  <input type="text" name="DONA_CRVYR<%= i %>" value="" size="4" maxlength="4" style="text-align:center" disabled>
                </td><!--이월공제년도-->
                <td>
                  <input type="text" name="DONA_DEDPR<%= i %>" value="" size="10"  maxlength="11"  onKeyUp="javascript:moneyChkEventForWon(this);" style="text-align:right" disabled>
                </td><!--전년까지공제액-->
             <% }else{ %>
                  <input type="hidden" name="DONA_CRVIN<%=i%>" value="">
                  <input type="hidden" name="DONA_CRVYR<%=i%>" value="">
    ...
    <div class="buttonArea">
        <ul class="btn_crud">
            <li><a class="darken" href="javascript:do_build();"><span><spring:message code="LABEL.D.D11.0073" /><!-- 입력 --></span></a>
        </ul>
    </div>
