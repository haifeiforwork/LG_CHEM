...
/*   2Depth Name  : 연말정산                                                    */
/*   Program Name : 연말정산                                                    */
/*   Program ID   : D11TaxAdjustCardBuild.jsp                                   */
...
<%--  [CSR ID:3569665]   <%if(tab_gubun.equals("1")){//신용카드(1)%>
    for( var i = 0 ; i < "<%= card_count %>" ; i++ ) {
        ...
        //exprd = eval("document.form1.EXPRD_"+i+".value");  [CSR ID:3569665]
...
<%if(tab_gubun.equals("1")){//신용카드(1)%>
            ...
            if( eval("document.form1.OMIT_FLAG"+ i + ".checked == false") ){
                //if(subty=="35"&&exprd=="1"){[CSR ID:3569665]
                //if(subty=="35"&&exprd=="4"){[CSR ID:3569665]
                if(subty=="35"){
...
<%-- <%if(tab_gubun.equals("1")){//신용카드(1)%>
			 /* [CSR ID:3569665] 2017년 연말정산 웹화면 수정 요청의 건  start @2017 사용기간 삭제로 주석처리 */
...
<%}%> --%>
            ...
            if( eval("document.form1.BETRG_O"+ i + ".value == ''") )
             ...
             eval("document.form1.TRDMK"+i+".disabled  = false;");
...
function subty_change(row, obj) {
    ...
    <%  if (tab_gubun.equals("1")) { //신용카드/현금 %>
    eval("document.form1.CHNTS"+row+".checked = false;");
    //eval("document.form1.EXPRD_"+row+".selectedIndex = 0;");//@2014 연말정산[CSR ID:3569665]
    <% } %>
...
function do_check(row, obj) {
    ...
    if ( eval("document.form1.E_GUBUN"+row+".value;")=="5" ) { //지로영수증
         ...
         eval("document.form1.CCTRA"+ row + ".checked = false"); //CSR ID:2013_9999 대중교통
         eval( "document.form1.CCTRA"+row+".disabled  = true;"); //CSR ID:2013_9999 대중교통
    }else{
         eval( "document.form1.TRDMK"+row+".disabled  = false;"); //C20121213_34842 2012
         eval( "document.form1.CCTRA"+row+".disabled  = false;"); //CSR ID:2013_9999 대중교통
    }
    <% } %>
...
function check_TrdmkCctra(obj,gubn, row) {
   //CSR ID:2013_9999 전통시장과 대중교통중 한가지만 체크
...
/* [CSR ID:3569665] 2017년 연말정산 웹화면 수정 요청의 건  start @2017연말정산 사용기간 삭제로 주석처리 */
            ...
            <table class="listTable" id="table">
              <thead>
                <tr>
                    ...
                    <th><spring:message code="LABEL.D.D11.0048" /><!-- 공제대상액 --></th>
                    <th><spring:message code="LABEL.D.D11.0049" /><!-- 전통시장 --></th>
                    <th><spring:message code="LABEL.D.D11.0050" /><!-- 대중교통 --></th>
                    <!-- [CSR ID:3569665] 2017년 연말정산 웹화면 수정 요청의 건  start @2017 사용기간 삭제로 주석처리-->
                    <%-- <th><spring:message code="LABEL.D.D11.0051" /><!-- 사용기간 --></th> --%><!-- @2014 연말정산 -->
					<!-- [CSR ID:3569665] 2017년 연말정산 웹화면 수정 요청의 건  end-->
                    <th><spring:message code="LABEL.D.D11.0052" /><!-- 회사비용정리금 --></th>
                    <th width=""><spring:message code="LABEL.D.D11.0053" /><!-- 국세청자료 --></th>
                    <% if("Y".equals(pdfYn)) {%>
...
%>
          <tr class="<%=tr_class%>">
            ...
            <td><!--@v1.4--><input type="radio" name="radiobutton" value="<%=i%>" <%= data.GUBUN.equals("9") ? "disabled" : "" %>></td>
            <td>
...
<%= WebUtil.printOption((new D11FamilyRelationRFC()).getFamilyRelation(""), data.SUBTY) %>
            ...
            <td >
              <input type="hidden" name="iF_ENAME<%=i%>" value="<%= data.F_ENAME.equals("") ? "" : data.F_ENAME %>" size="8" class="input03" maxlength="20">
...
%>
            ...
            <td>
              <input type="text" name="F_REGNO<%=i%>" value="<%= data.F_REGNO.equals("") ? "" : DataUtil.addSeparate(data.F_REGNO) %>" size="13" readonly maxlength="14" onBlur="javascript:return;resno_chk(this);" disabled>
            </td>
            <td>
              <select name="E_GUBUN<%=i%>" class="<%= data.GUBUN.equals("9")  ? "input04" : "input03" %>" onChange="javascript:do_check(<%=i%>,this);">
                <option value="">-------------</option>
                ...
                <!--<option value="5" <%= data.E_GUBUN.equals("5") ? "selected" : "" %>>지로영수증</option> CSR ID :2013_9999-->
                <option value="6" <%= data.E_GUBUN.equals("6") ? "selected" : "" %>><spring:message code="LABEL.D.D11.0057" /><!-- 직불/선불카드 --></option>
              </select>

            </td>
            <td>
              <input type="text" name="BETRG<%=i%>" value="<%= data.BETRG.equals("")  ? "" : WebUtil.printNumFormat(data.BETRG) %>" size="12" maxlength="11" class="<%= data.GUBUN.equals("9")  ? "input04" : "input03" %>" onKeyUp="javascript:moneyChkEventForWon(this);" style="text-align:right"  <%=BETRG_Display%> <%= data.GUBUN.equals("9") ? "disabled" : "" %>>
            </td>
            <!--C20121213_34842 2012 전통시장여부 -->
            <td>
              <input type="checkbox" name="TRDMK<%=i%>" value="<%=  data.TRDMK.equals("") ? "" : data.TRDMK %>" <%= data.TRDMK.equals("X")  ? "checked" : "" %> onClick="javascript:check_TrdmkCctra(this,'1',<%=i%>);" <%= data.GUBUN.equals("9") ? "disabled" : "" %>>
            </td>
            <!--CSR ID:2013_9999 대중교통추가 -->
            <td>
              <input type="checkbox" name="CCTRA<%=i%>" value="<%=  data.CCTRA.equals("") ? "" : data.CCTRA %>" <%= data.CCTRA.equals("X")  ? "checked" : "" %> onClick="javascript:check_TrdmkCctra(this,'2',<%=i%>);" <%= data.GUBUN.equals("9") ? "disabled" : "" %>>
            </td>
            <!-- @2014 연말정산 사용기간 text/code추가  -->
            <!-- [CSR ID:3569665] 2017년 연말정산 웹화면 수정 요청의 건  start @2017 사용기간 삭제로 주석처리-->
            <%-- <td>
             <select name="EXPRD_<%= i %>" onchange="check_UseCardPeriod(this,<%=i%>);">
             <option value="">-------------</option>
                <%= WebUtil.printOption((new D11TaxAdjustCardUseYearComboRFC()).getCardUseYearCombo(targetYear)) %>
            </select>
            <input type="hidden" name="EXSTX_<%= i %>" value="" size="30" maxlength=100 >
          </td> --%>
          	<!-- [CSR ID:3569665] 2017년 연말정산 웹화면 수정 요청의 건  end -->
            <td>
              <input type="text" name="BETRG_B<%=i%>" value="<%= data.BETRG_B.equals("")  ? "" : WebUtil.printNumFormat(data.BETRG_B) %>" size="12" maxlength="11" class="<%= data.GUBUN.equals("9")  ? "input04" : "input03" %>" onKeyUp="javascript:moneyChkEventForWon(this);" style="text-align:right"  disabled>
            </td>

            <td>
              <input type="checkbox" name="CHNTS<%=i%>" value="<%= data.CHNTS.equals("") ? "" : data.CHNTS %>" <%= data.CHNTS.equals("X") ? "checked" : "" %> <%= data.GUBUN.equals("9") ? "disabled" : "" %>>
            </td>
            <% if("Y".equals(pdfYn)) {%>
            <td>
...
<%= WebUtil.printOption((new D11FamilyRelationRFC()).getFamilyRelation(""), "") %>
            ...
            <td>
              <input type="hidden" name="iF_ENAME<%=i%>" value="" size="8" maxlength="20" >
...
%>
            ...
            <td><input type="text" name="F_REGNO<%=i%>" value="" size="13" readonly maxlength="14" onBlur="javascript:return;resno_chk(this);" disabled></td>
            <td>
              <select name="E_GUBUN<%=i%>" onChange="javascript:do_check(<%=i%>,this);">
                <option value="">-------------</option>
                ...
                <!--<option value="5">지로영수증</option> CSR ID :2013_9999-->
                <option value="6"><spring:message code="LABEL.D.D11.0057" /><!-- 직불/선불카드 --></option>
              </select>
            </td>
            <td><input type="text" name="BETRG<%=i%>" value="" size="12" maxlength="11" onKeyUp="javascript:moneyChkEventForWon(this);" style="text-align:right" ></td>
            <!--C20121213_34842 2012 전통시장여부 -->
            <td><input type="checkbox" name="TRDMK<%=i%>" value="" onClick="javascript:check_TrdmkCctra(this,'1',<%=i%>);"></td>
            <!--CSR ID:2013_9999 대중교통추가 -->
            <td><input type="checkbox" name="CCTRA<%=i%>" value="" onClick="javascript:check_TrdmkCctra(this,'2',<%=i%>);"></td>
            <!-- @2014 연말정산 사용기간 text/code추가  -->
            <!-- [CSR ID:3569665] 2017년 연말정산 웹화면 수정 요청의 건  start @2017 사용기간 삭제로 주석처리-->
            <%-- <td>
             <select name="EXPRD_<%= i %>" onchange="check_UseCardPeriod(this,<%=i%>);">
            ...
            <input type="hidden" name="EXSTX_<%= i %>" value="" size="30" maxlength=100>
          	...
          	<!-- [CSR ID:3569665] 2017년 연말정산 웹화면 수정 요청의 건  end-->
            <td><input type="text" name="BETRG_B<%=i%>" value="" size="12" maxlength="11" onKeyUp="javascript:moneyChkEventForWon(this);" style="text-align:right" disabled></td>

            <td><input type="checkbox" name="CHNTS<%=i%>" value=""></td>

            <% if("Y".equals(pdfYn)) {%>
            <td><input type="checkbox" name="GUBUN<%=i%>" value="" disabled></td>
            <td class="lastCol"><input type="checkbox" name="OMIT_FLAG<%=i%>" value="" disabled></td>
            <%} else {%>
                <div style="display:none">
                <input type="checkbox" name="GUBUN<%=i%>">
                <input type="checkbox" name="OMIT_FLAG<%=i%>">
        ...
        <div class="table">
            ...
            <thead>
                <tr>
                    ...
                    <th><spring:message code="LABEL.D.D11.0053" /><!-- 국세청자료 --></th>
                    <% if("Y".equals(pdfYn)) {%>
                    <th>PDF</th>
                    <th class="lastCol"><spring:message code="LABEL.D.D11.0055" /><!-- 연말정산삭제 --></th>
                    <%} %>
...
<%= WebUtil.printOption((new D11FamilyRelationRFC()).getFamilyRelation(""), data.SUBTY) %>
            ...
            <td>
              <input type="hidden" name="iF_ENAME<%=i%>" value="<%= data.F_ENAME.equals("") ? "" : data.F_ENAME %>">
              <select name="F_ENAME<%=i%>" class="<%= data.GUBUN.equals("9")  ? "input04" : "input03" %>" onChange="javascript:subty_change(<%=i%>,this);" <%= data.GUBUN.equals("9") ? "disabled" : "" %>>
                <option value="">--------</option>
<%
    //@v1.3
...
%>
            ...
            <td>
              <input type="text" name="BETRG<%=i%>" value="<%= data.BETRG.equals("")  ? "" : WebUtil.printNumFormat(data.BETRG) %>" size="15" maxlength="11" class="<%= data.GUBUN.equals("9")  ? "input04" : "input03" %>" onKeyUp="javascript:moneyChkEventForWon(this);" style="text-align:right"  <%=BETRG_Display%> <%= data.GUBUN.equals("9") ? "disabled" : "" %>>
            </td>
            <td>
              <input type="checkbox" name="CHNTS<%=i%>" value="<%= data.CHNTS.equals("") ? "" : data.CHNTS %>" <%= data.CHNTS.equals("X") ? "checked" : "" %> <%=  data.E_GUBUN.equals("2")||data.GUBUN.equals("9") ? "disabled" : "" %>>
            </td>
            <% if("Y".equals(pdfYn)) {%>
            <td>
             <input type="checkbox" name="GUBUN<%=i%>" value="<%=  data.GUBUN.equals("9") ? "X" : "" %>" <%= data.GUBUN.equals("9")  ? "checked" : "" %> disabled>
            </td>
            <td class="lastCol">
            <input type="checkbox" name="OMIT_FLAG<%=i%>" value="<%=  data.OMIT_FLAG.equals("") ? "" : data.OMIT_FLAG %>" <%= data.OMIT_FLAG.equals("X")  ? "checked" : "" %> class="input03" <%= !data.GUBUN.equals("9") ? "disabled" : "" %>>
            </td>
...
%>
            ...
            <td>
              <input type="text" name="F_REGNO<%=i%>" value="" size="13" readonly maxlength="14" onBlur="javascript:return;resno_chk(this);" disabled>
            </td>
            <td>
            ...
            <td>
              <input type="text" name="BETRG<%=i%>" value="" size="15" maxlength="11" onKeyUp="javascript:moneyChkEventForWon(this);" style="text-align:right" >
            </td>
            <td>
              <input type="checkbox" name="CHNTS<%=i%>" value="">
            </td>
            <% if("Y".equals(pdfYn)) {%>
            <td>
