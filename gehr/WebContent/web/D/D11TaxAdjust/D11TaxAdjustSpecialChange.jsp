...
/*   2Depth Name  : 연말정산                                                    */
/*   Program Name : 연말정산                                                    */
/*   Program ID   : D11TaxAdjustSpecialChange.jsp                               */
...
function do_change() {
    for( var i = 0 ; i < "<%= special_vt.size() %>" ; i++ ) {
        ...
        omit_flag = eval("document.form1.OMIT_FLAG"+i+".value");
        ...
        if ( eval("document.form1.GOJE_CODE"+i+".value == '19200000'") ||
        ...
        } else if ( goje_flag == "1" && omit_flag != "X") { //삭제자료는 체크 안함
            ...
            if (betrg!="" && omit_flag!="X" &&  req_h=="X"  && document.form1.FSTID.checked != true ) {
...
%>
            <td  style="text-align:right">
              <select name="betrg<%= i %>" class="<%= data.GUBUN.equals("9") ? "input04" : "input03" %>" style="text-align:right" <%= data.GUBUN.equals("9") ? "disabled" : "" %>>
<%
            for( int x = 0 ; x < 11 ; x++ ) {
                String cntx     = Integer.toString(x);
                String addbetrg = WebUtil.printNumFormat(data.ADD_BETRG, 0 );

%>
                <option value="<%= x %>" <%= addbetrg.equals(cntx) ? " selected " : "" %> ><%= x %></option>
<%
            }
%>
            ...
            <td  style="text-align:left">&nbsp;<%= data.AUTO_TEXT %></td>
<%
        } else {
            if( data.GOJE_FLAG.equals("1") ) {
%>
            <td >
              <input type="text" name="betrg<%= i %>" value="<%= data.ADD_BETRG.equals("0.0")  ? "" : WebUtil.printNumFormat(data.ADD_BETRG) %>" size="15" maxlength="11" class="class="<%= data.GUBUN.equals("9") ? "input04" : "input03" %>" onKeyUp="javascript:moneyChkEventForWon(this);" style="text-align:right" onFocus="this.select()" <%= data.GUBUN.equals("9") ? "disabled" : "" %>>
            </td>
<%
            } else {
%>
...
%>
            ...
            <% if("@".equals(pdfYn)) {%><!-- @2015 연말정산 사용안함. -->
            <td ><!--연말정산삭제-->
            <input type="checkbox" name="OMIT_FLAG<%=i%>" value="<%=  data.OMIT_FLAG.equals("") ? "" : data.OMIT_FLAG %>" <%= data.OMIT_FLAG.equals("X")  ? "checked" : "" %> class="input03" <%= !data.GOJE_FLAG.equals("9") ? "disabled" : "" %>>
            </td>
            <%} else {%>
