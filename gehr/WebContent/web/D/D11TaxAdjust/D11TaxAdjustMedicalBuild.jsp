...
/*   2Depth Name  : 연말정산                                                    */
/*   Program Name : 연말정산                                                    */
/*   Program ID   : D11TaxAdjustMedicalBuild.jsp                                */
        ...
        for( var i = 0 ; i < "<%= medi_count %>" ; i++ ) {
            ...
            if( eval("document.form1.GUBUN"+ i + ".checked == true") ) {
                ...
                if(eval("document.form1.PDF"+ i + ".checked == true")) {
                    eval("document.form1.GUBUN"+ i + ".value ='9';");
                } else {
...
function subty_change(row, obj) {
    ...
    if( inx > 0 ) {
        ...
        eval( "document.form1.DIFPG_CHK"+row+".value = \"\";");//@2015연말정산
...
<%= WebUtil.printOption((new D11FamilyRelationRFC()).getFamilyRelation(""), data.SUBTY) %>
            ...
            <td>
              ...
              <select name="F_ENAME<%=i%>"  style="width=60px" auto onChange="javascript:subty_change(<%=i%>,this);checkAgeField(<%=i%>);">
                <option value="">--------</option>
<%
    //@v1.3
    old_Name = "";
...
%>
            ...
            <td>
              <input type="text" name="F_REGNO<%=i%>" value="<%= data.F_REGNO.equals("") ? "" : DataUtil.addSeparate(data.F_REGNO) %>" size="13" readonly maxlength="14" onBlur="javascript:return;resno_chk(this);">
            </td>
            <td>
              <input type="text" name="CONTENT<%=i%>" value="<%= data.CONTENT.equals("") ? "" : data.CONTENT %>" size="13" maxlength="20"  <%= data.GUBUN.equals("1")||data.METYP.equals("1") ? "disabled" : "" %>>
            </td>
            <td nowrap>
            <select style="width=80px" auto  id="d_METYP<%=i%>" name="METYP<%= i %>"  onChange="javascript:metyp_change(<%=i%>,this);">
             <option value="">-------------</option>
    <%= WebUtil.printOption((new D11TaxAdjustMedicalTypeRFC()).getMedicalType(), data.METYP) %>
            </select></td><!--코드 CSR ID:1361257-->
            <td>
              <input type="checkbox" name="GLASS_CHK<%=i%>" value="" onclick="noDup('1','<%=i%>');">
            </td><!--안경콘택트-->
            <td>
              <input type="checkbox" name="DIFPG_CHK<%=i%>" value="" onclick="noDup('2','<%=i%>');">
            </td><!--@2015연말정산 난임시술비-->
            <td>
              <input type="text" name="BIZNO<%=i%>" value="<%= data.BIZNO.equals("") ? "" : DataUtil.addSeparate2(data.BIZNO) %>" size="12" maxlength="12" onBlur="javascript:name_search(this,<%=i%>);businoFormat(this);return;usableChar(this, '0123456789');" <%= data.GUBUN.equals("1")||data.METYP.equals("1") ? "disabled" : "" %>>
            </td>
            <td>
              <input type="text" name="BIZ_NAME<%=i%>" value="<%= data.BIZ_NAME.equals("") ? "" : data.BIZ_NAME %>" size="13" maxlength="20" <%= data.GUBUN.equals("1")||data.METYP.equals("1") ? "disabled" : "" %>>
            </td>
            <td>
              <input type="text" name="CA_CNT<%=i%>" value="<%= data.CA_CNT.equals("") ? "" : Integer.toString(Integer.parseInt(data.CA_CNT)) %>" size="3" style="text-align:right" maxlength="5"  onBlur="javascript:usableChar(this, '0123456789');" <%= data.GUBUN.equals("1")||data.METYP.equals("1") ? "disabled" : "" %>>
            </td>
            <td>
              <input type="text" name="CA_BETRG<%=i%>" value="<%= data.CA_BETRG.equals("")  ? "" : WebUtil.printNumFormat(data.CA_BETRG) %>" size="10"  maxlength="11" onKeyUp="javascript:moneyChkEventForWon(this);" style="text-align:right" >
            </td>
            <td>
              <input type="checkbox" name="OLDD<%=i%>" value="<%= data.OLDD.equals("") ? "" : data.OLDD %>" <%= data.OLDD.equals("X") ? "checked" : "" %>>
            </td>
            <td>
              <input type="checkbox" name="OBST<%=i%>" value="<%= data.OBST.equals("") ? "" : data.OBST %>" <%= data.OBST.equals("X") ? "checked" : "" %>>
            </td>
            <!--  회사지원분삭제 2017.1.6 김지수k요청 -->
            ...
            <%} %>
            <td class="lastCol">
              <input type="checkbox" name="OMIT_FLAG<%=i%>" value="<%=  data.OMIT_FLAG.equals("") ? "" : data.OMIT_FLAG %>" <%= data.OMIT_FLAG.equals("X")  ? "checked" : "" %> disabled>
            </td>
          </tr>
<%
    }
...
%>
            ...
            <td>
              <input type="text" name="F_REGNO<%=i%>" value="" size="13" readonly maxlength="14" onBlur="javascript:return;resno_chk(this);" >
            </td>
            <td>
              <input type="text" name="CONTENT<%=i%>" value="" size="13" maxlength="20">
            </td>
            <td nowrap>
            <select style="width=80px" auto  id="d_METYP<%=i%>" name="METYP<%= i %>"  onChange="javascript:metyp_change(<%=i%>,this);">
             <option value="">-------------</option>
    <%= WebUtil.printOption((new D11TaxAdjustMedicalTypeRFC()).getMedicalType(), "") %>
            ...
            <td>
              <input type="text" name="BIZNO<%=i%>" value="" size="12" maxlength="12" onBlur="javascript:name_search(this,<%=i%>);businoFormat(this);return;usableChar(this, '0123456789');">
            </td>
            <td>
              <input type="text" name="BIZ_NAME<%=i%>" value="" size="13" maxlength="20">
            </td>
            <td>
              <input type="text" name="CA_CNT<%=i%>" value="" size="3" style="text-align:right" maxlength="5" onBlur="javascript:usableChar(this, '0123456789');">
            </td>
            <td>
              <input type="text" name="CA_BETRG<%=i%>" value="" size="10" maxlength="11" onKeyUp="javascript:moneyChkEventForWon(this);" style="text-align:right" >
            </td>
            <td>
              <input type="checkbox" name="OLDD<%=i%>" value="">
            </td>
            <td>
              <input type="checkbox" name="OBST<%=i%>" value="">
            </td>
            <!--  회사지원분삭제 2017.1.6 김지수k요청 -->
            <td style="display:none"><!--@v1.4-->
            ...
            <td>
              <input type="checkbox" name="CHNTS<%=i%>" value="" disabled>
            </td>
            <% if("Y".equals(pdfYn)) {%>
            <td>
                <input type="checkbox" name="PDF<%=i%>" value="" disabled>
            </td>
            <%} else {%>
                <div style="display:none">
                <input type="checkbox" name="PDF<%=i%>">
...
<!-- 숨겨진 필드 -->
  ...
  <input type="hidden" name="aaaa" value="<%=medi_vt.toString()%>">
<!-- 숨겨진 필드 -->
