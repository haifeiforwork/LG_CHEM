...
/*   2Depth Name  : 연말정산                                                    */
/*   Program Name : 연말정산                                                    */
/*   Program ID   : D11TaxAdjustTaxChange.jsp                                   */
...
function do_change() {
    ...
    if ( betrg_sum >  12000000) {
       alert("<spring:message code='MSG.D.D11.0099' />"); //장기주식형저축소득공제금액은 연간 1200만원 이내로 입력해야 합니다.
       return;
    }
...
%>
            <td >
              <input type="text" name="betrg<%= i %>" value="<%= data.ADD_BETRG.equals("0.0")  ? "" : WebUtil.printNumFormat(data.ADD_BETRG) %>" size="15" maxlength="11" class="input03" onKeyUp="javascript:moneyChkEventForWon(this);" style="text-align:right" onFocus="this.select()" maxlength="11">
            </td>
<%
        } else {
%>
