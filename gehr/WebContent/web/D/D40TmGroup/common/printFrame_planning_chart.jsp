...
<%
	...
	String empNo = WebUtil.nvl(request.getParameter("I_PERNR"));
	String empNm = WebUtil.nvl(request.getParameter("I_ENAME"));

	String I_DATUM = WebUtil.nvl(request.getParameter("I_DATUM")).replace(".","");
...
//     targetPage = WebUtil.ServletURL+"hris.F.F42DeptMonthWorkConditionSV?hdn_deptId="+hdn_deptId+",hdn_excel="+hdn_excel+",year1="+year1+",month1="+month1+",chck_yeno="+checkYn;
    targetPage = WebUtil.ServletURL+"hris.D.D40TmGroup.D40TmSchkzPlanningChartSV?gubun="+gubun+",orgOrTm="+orgOrTm+",searchDeptNo="+searchDeptNo+",searchDeptNm="+searchDeptNm+
    		",iSeqno="+iSeqno+",I_SELTAB="+I_SELTAB+",empNo="+empNo+",empNm="+empNm+",I_DATUM="+I_DATUM+",I_ENDDA="+I_ENDDA+",ISEQNO="+ISEQNO;
    if( isPopUp == null ) {
        isPopUp = "";
    }
