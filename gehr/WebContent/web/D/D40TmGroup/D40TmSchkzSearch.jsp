...
%>
	...
	%>
			var selectList = '<select name="SCHKZ"><option value="">전체</option>';
			var code;
			var text;
	<%
	...
	$(function() {
		...
		$("#do_save, #save").click(function(){
			...
			$("#gubun").val("SAVE");
			$("#form1").attr("action","<%= WebUtil.ServletURL %>hris.D.D40TmGroup.D40TmSchkzSearchSV");
...
%>
						<tr class="<%=tr_class%>">
<%-- 							<td><%=i+1 %></td> --%>
							<td><%=WebUtil.printString( data.PERNR )%><input type="hidden" name="PERNR" value="<%=data.PERNR %>"></td>
							<td><%=WebUtil.printString( data.ENAME )%><input type="hidden" name="ENAME" value="<%= data.ENAME %>"></td>
							<td><%=WebUtil.printString( data.BEGDA ).replace("-",".")%><input type="hidden" name="BEGDA" value="<%= data.BEGDA.replace("-",".")%>"></td>
							<td><%=WebUtil.printString( data.ENDDA ).replace("-",".")%><input type="hidden" name="ENDDA" value="<%= data.ENDDA.replace("-",".")%>"></td>
							<%-- <td><%=data.SCHKZ%></td> --%>
							<td>
								<%=WebUtil.printString(data.SCHKZ)%>
							...
							<td>
								<select name="NSCHKZ" style="width: 90px;" >
...
%>
						...
						<%if("SAVE".equals(gubun)){ %>
							<td class="lastCol" style="text-align : left"><%=WebUtil.printString(data.MSG) %></td>
						<%}else{ %>
							<td class="lastCol" style="text-align : left"><%=WebUtil.printString(data.ETC) %></td>
						<%} %>
						</tr>
<%
			} //end for
