...
/*   Program ID		: D40AbscTimeEach.jsp									*/
/*   Description		: 비근무,근무(개별) 조회/저장							*/
/*   Note				:             													*/
	...
	function makeInput(){
		...
		$.each( arrPERNR, function( i, val ) {
			for(var k = 0; k < repeatCnt; k++) {
				...
				var select2 = "";
				if(D_REASON_YN == ""){
					select2 = defultSelectList2(rowCnt);
				}else{
					select2 = defultSelectList2(rowCnt, D_WTMCODE, D_REASON, D_REASON_YN);
				}
				...
				html += '			<input type="hidden" id="OAWART'+rowCnt+'" name="OAWART" value="">';
				html += '			<input type="hidden" id="OINFTY'+rowCnt+'" name="OINFTY" value="">';
			...
			if(D_DETAIL_YN == "Y" || D_DETAIL_YN == ""){
				html += '			<input type="text" id="DETAIL'+rowCnt+'" name="DETAIL" value="'+D_DETAIL+'" style="width: 44%; margin-bottom: 6px;">';
			}else{
				html += '			<input type="text" id="DETAIL'+rowCnt+'" name="DETAIL" value="'+D_DETAIL+'" disabled="disabled" style="width: 44%; margin-bottom: 6px;">';
			}
				html += '		</td>';
		...
		$("input[name=PERNR]").each(function(idx){
			...
			if($("#ACTIO"+no).val() != "DEL"){ //삭제 플래그는 validation 제외한다
				...
				if($.trim($("#WTMCODE"+no).val()) == ""){
					alert(target+'<spring:message code="MSG.D.D40.0030"/>');	/* 의 유형선택은 필수 입니다. */
					$("#WTMCODE"+no).focus();
					chk = false;
					return false;
				}
				...
				if($.trim($("#ENDDA"+no).val()) == ""){	/* 종료일은 필수 입니다. */
...
<%
		for( int i = 0; i < vt3.size(); i++ ) {
			D40AbscTimeFrameData data = (D40AbscTimeFrameData)vt3.get(i);
%>
		if(value1 == '<%=data.PKEY%>'){
			code = '<%=data.CODE%>';
			text = '<%=data.TEXT%>';
...
%>
	...
	//유형선택 select
	function defultSelectList1(index, val){
...
<%
			for( int j = 0; j < vt4.size(); j++ ) {
				D40AbscTimeFrameData vtData = (D40AbscTimeFrameData)vt4.get(j);
%>
				code = '<%=vtData.CODE%>';
				text = '<%=vtData.TEXT%>';
				if(code == val){
...
<%-- 			selectList += '<option value="<%=vtData.CODE%>" ><%=vtData.CODE%> (<%=vtData.TEXT %>)</option>'; --%>
	...
	$(function() {
		...
		$("#do_search").click(function(){
			...
			if(dt > 31){
				...
				$("#form1").attr("action","<%= WebUtil.ServletURL %>hris.D.D40TmGroup.D40AbscTimeEachSV");
		...
		$("#addRow").click(function(){
			...
			html += '		<td style="text-align: left;" id="td'+rowCnt+'">';
			html += 			select2;
			html += '			<input type="text" id="DETAIL'+rowCnt+'" name="DETAIL" value="" style="width: 44%; margin-bottom: 6px;">';
			html += '		</td>';
			html += '		<td id="td_REASON'+rowCnt+'"" style="display: none;"></td>';
			html += '		<td id="td_DETAIL'+rowCnt+'"" style="display: none;"></td>';
			html += '		<td id="td_EDIT'+rowCnt+'" style="display: none;">';
		...
		$("#copyRow").click(function(){
			...
			if(!dbClick){
				...
				if(D_TIME_YN == "Y" || D_TIME_YN == ""){
					...
					html += '		<td><input type="text" class="eTime" id="BEGUZ'+rowCnt+'" name="BEGUZ" value="'+D_BEGUZ+'" disabled="disabled" style="width: 35px;" maxlength="4"></td>';
					html += '		<td><input type="text" class="eTime" id="ENDUZ'+rowCnt+'" name="ENDUZ" value="'+D_ENDUZ+'" disabled="disabled" style="width: 35px;" maxlength="4"></td>';
					html += '		<td id="td_TIME'+rowCnt+'"" style="display: none;"><input type=hidden id="BEGUZ'+rowCnt+'" name="BEGUZ"  value="'+D_BEGUZ+'" ><input type=hidden id="ENDUZ'+rowCnt+'" name="ENDUZ"  value="'+D_ENDUZ+'" ></td>';
				}
					html += '		<td style="text-align: left;" id="td'+rowCnt+'">';
					html +=			select2;
				if(D_DETAIL_YN == "Y" || D_DETAIL_YN == ""){
...
<%
					if( vt2 != null && vt2.size() > 0 ){
						for( int i = 0; i < vt2.size(); i++ ) {
							D40AbscTimeFrameData data = (D40AbscTimeFrameData)vt2.get(i);
					        String BEGUZ = "";
...
%>
						<tr class="<%=WebUtil.printOddRow(i)%>"  id="tr<%=i+1 %>">
							...
							<%} %>
								...
								<input type="hidden" name="OAWART"  value="<%=data.OAWART%>">
								<input type="hidden" name="OINFTY"  value="<%=data.OINFTY%>">
...
%>
						...
						<%if("X".equals(WebUtil.nvl(EDIT))){ %>
								<% if(!"0000-00-00".equals(data.ENDDA)){ %>
									<input type="text"  class="date" id="ENDDA<%=i+1 %>" name="ENDDA" value="<%=data.ENDDA.replace("-",".")%>" size="15" onBlur="javascript:dateFormat(this);" style="margin-right: 4px">
								<%}else{ %>
						...
						<%}else{ %>
								<% if(!"0000-00-00".equals(data.ENDDA)){ %>
									<input type="text"  id="ENDDA<%=i+1 %>" name="ENDDA" disabled="disabled" value="<%=data.ENDDA.replace("-",".")%>"  style="width: 105px; ">
								<%}else{ %>
									<input type="text"  id="ENDDA<%=i+1 %>" name="ENDDA" disabled="disabled" value=""   style="width: 105px; ">
								<%} %>
...
<%
									for( int j = 0; j < vt3.size(); j++ ) {
										D40AbscTimeFrameData vtData = (D40AbscTimeFrameData)vt3.get(j);
...
%>
						...
						<%if(!"Y".equals(data.TIME_YN) || !"X".equals(EDIT)){ %>
