...
/*   Program ID   : E13CyHospitalcalendar.jsp                                    */
/*   Description  : 종합검진 신청현황달력                                       */
/*   Note         :                                                             */
/*   Creation     : 2008-01-18  lsa                                             */
/*   Update       :   2016-03-15 //[CSR ID:3004032] 여수 site에 한하여 alert 추가                                                          */
/*                                                                              */
/********************************************************************************/%>
...
<!--
  ...
  function submit_date(selectDate,reserv,reserv1  ) {
      ...
      if (reserv1 =="0000" && "<%=StringEscapeUtils.escapeHtml(STMCCHECK)%>" !="" ) {
...
//[CSR ID:3004032] 여수 site에 한하여 alert 추가
  ...
  function Submitit(){
    ...
    url = '<%=WebUtil.JspURL%>E/E13CyGeneral/E13CyHospitalcalendar.jsp?PERNR=<%=PERNR%>&moreScriptFunction=<%=moreScriptFunction%>&optionvalue=selectvalue';
  ...
  function PrevNext(val){
    url = '<%=WebUtil.JspURL%>E/E13CyGeneral/E13CyHospitalcalendar.jsp?PERNR=<%=PERNR%>&moreScriptFunction=<%=moreScriptFunction%>';
	url +='&optionvalue='+val;
	url +='&formname=<%=request.getParameter("formname")%>';
	url +='&fieldname=<%=request.getParameter("fieldname")%>&curDate=<%=curDate %>&iflag=2';
