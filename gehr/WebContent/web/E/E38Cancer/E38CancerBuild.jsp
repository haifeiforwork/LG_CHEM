...
/*   Program Name : 추가암검진(7대암검진) 신청                                               */
/*   Program ID   : E38CancerBuild.jsp                                         */
/*   Description  : 추가암검진(7대암검진)을 신청할 수 있는 화면                              */
/*   Note         :                                                             */
...
<tags-approval:request-layout  titlePrefix="COMMON.MENU.ESS_BE_ADDI_CHECK" >
	<tags:script>
	<script>
	<!--
	...
	function sel_hidden(gubun) {
	    ...
	    document.form2.action = "<c:out value='${g.jsp}'/>E/E38Cancer/E38CancerHiddenSelect.jsp";
	...
	function fn_openCal(Objectname){
		...
		<c:forEach var="row" items="${E38CancerHospCodeData_opt}" varStatus="inx">
		...
		//if (HOSP_CODE ==  "<c:out value='${row.HOSP_CODE}'/>" && "<c:out value='${row.STMC_CODE1}'/>}" == STMC_CODE) {
	     ...
	     //[CSR ID:3393141] HR종합검진 ERP시스템 에러개선 요청 end
		  ...
		  small_window  = window.open("<c:out value='${g.jsp}'/>E/E38Cancer/E38CancerHospitalcalendar.jsp?formname=form1&fieldname="+ Objectname + "&curDate=" + lastDate + "&iflag=0&PERNR=<c:out value='${PERNR}'/>"+STMCCHECK+"&HOSP_CODE="+document.form1.HOSP_CODE.value,"essCal", "toolbar=0,location=0,status=0,menubar=0,resizable=0,width="+sWidth+",height="+sHeight+",left="+ml+",top="+mt);
		  small_window.focus();
	}
		...
		<div class="table">
			<table class="tableGeneral">
            	...
            	<tr>
	                ...
	                <th class="th02"><span class="textPink">*</span><!--구분--><spring:message code="LABEL.E.E28.0002" /></th>
	                <td>
	                	<select name="GUEN_CODE" style="width:135px;" >
	                    <option value="0001"><!--본인--><spring:message code="LABEL.E.E28.0018" /></option>
	                    </select>
                    </td>
                </tr>
               ...
               <tr>
                   ...
                   <th class="th02"><span class="textPink">*</span><!--소화기검사선택--><spring:message code="LABEL.E.E28.0010" /></th>
                   <td>
                       <select name="STMC_CODE" style="width:135px;" onChange="javascript:sel_hidden('STMC_CODE');">

                       <c:choose>
                       		<c:when test="${ fn:length(E38CancerStmcData_opt) eq 0 }">
                       ...
                       <c:if test="${HOSP_CODE ne '' }" >
	                       <c:forEach var="row" items="${E38CancerStmcData_opt}" varStatus="inx">
	                       <c:set var="index" value="${inx.index}"/>
                ...
                <tr>
                    ...
                    <th class="th02"><span class="textPink">*</span><!--선택검사--><spring:message code="LABEL.E.E28.0012" /></th>
                    <td>
                    	<select name="SELT_CODE" style="width:135px;" onChange="javascript:sel_hidden('SELT_CODE');">

                    	<c:choose>
                       		<c:when test="${ fn:length(E38CancerSeltData_opt) eq 0 }">
								<option value=""><!--해당사항없음--><spring:message code="LABEL.E.E28.0013" /></option>
                       		</c:when>
                       		<c:otherwise>
								<option value="">-------------</option>
                        ...
                        <c:if test="${HOSP_CODE ne '' }" >
	                       <c:forEach var="row" items="${E38CancerSeltData_opt}" varStatus="inx">
	                       <c:set var="index" value="${inx.index}"/>
                ...
                <tr>
	                ...
	                <th class="th02"><span class="textPink">*</span><!--검진희망일--><spring:message code="LABEL.E.E28.0003" /></th>
	                <td>
	                	<input type="hidden" name="EZAM_DATE" value="">
	                	<input type="text" name="to_date" value="<c:out value='${EZAM_DATE}'/>" size="10" onBlur="javascript:dateFormat(this);" readonly />
                ...
                <!-- 여수,나주  -->
                 ...
                 <tr>
	                 ...
	                 <td colspan="3"><input type="text" name="BIGO" size="105" value="" >
	                 </td>
                 </tr>
                 </c:if>
		...
		<div class="table">
			<table class="tableGeneral">
            	...
            	<tr>
		         ...
		         <tr>
		              <td> <font color="black"><b>&nbsp;&nbsp;&nbsp;</b> : <spring:message code="MSG.E.E38.0002" /><!--Elian → HR center → 결재함 → 결재진행중인 문서 → 추가암검진(7대암검진) 삭제 클릭 후 재신청(신청방법 처음부터 다시 시작 함)--></font>
		              </td>
		         </tr>
	...
	<input type="hidden" name = "ZCONFIRM" value="<c:out value='${ZCONFIRM}'/>">
  	<input type="hidden" name = "ZCANCER" value="X"><!--추가추가암검진(7대암검진)(7종암) 여부-->
	<input type="hidden" name="HOSP_NAME" value="">
	<input type="hidden" name="GUEN_NAME" value="">
	<input type="hidden" name="M_FLAG" value="<c:out value='${E38Cancer.e_M_FLAG}'/>">
	<input type="hidden" name="F_FLAG" value="<c:out value='${E38Cancer.e_F_FLAG}'/>">
	<input type="hidden" name="ME_FLAG" value="<c:out value='${E38CancerFlagData.ME_FLAG}'/>">
	<input type="hidden" name="WI_FLAG" value="<c:out value='${E38CancerFlagData.WI_FLAG}'/>">
	<input type="hidden" name="ME_CODE" value="<c:out value='${E38CancerFlagData.ME_CODE}'/>">
	<input type="hidden" name="WI_CODE" value="<c:out value='${E38CancerFlagData.WI_CODE}'/>">
