...
/*   Program ID   : E38CancerHospitalcalendar.jsp                               */
/*   Description  : 종합검진 신청현황달력                                       */
/*   Note         :                                                             */
/*   Creation     : 2008-01-18  lsa  C20130620_53407                                           */
/*   Update       :                                                             */
/*                                                                              */
/********************************************************************************/%>
...
//처음일 경우 & 기본 날짜가 없을 경우..
   ...
   Vector E38CancerPossNumData_vt  = (new E38CancerPossNumRFC()).getInwonCnt(Pernr,Yymm,HOSP_CODE);
   ...
   for( int i = 0 ; i < E38CancerPossNumData_vt.size() ; i++ ) {
       E38CancerPossNumData data = (E38CancerPossNumData)E38CancerPossNumData_vt.get(i);
...
<!--
  ...
  function Submitit(){
    ...
    url = '<%=WebUtil.JspURL%>E/E38Cancer/E38CancerHospitalcalendar.jsp?PERNR=<%=PERNR%>&moreScriptFunction=<%=moreScriptFunction%>&optionvalue=selectvalue';
  ...
  function PrevNext(val){
    url = '<%=WebUtil.JspURL%>E/E38Cancer/E38CancerHospitalcalendar.jsp?PERNR=<%=PERNR%>&moreScriptFunction=<%=moreScriptFunction%>';
	url +='&optionvalue='+val;
	url +='&formname=<%=request.getParameter("formname")%>';
	url +='&fieldname=<%=request.getParameter("fieldname")%>&curDate=<%=curDate %>&iflag=2';
  ...
  <div class="tableInquiry">
        <table>
        	<tr>
        		<th>검진병원</th>
        		<td>
        			<select name="HOSP_CODE" class="input03"  onChange="Submitit();" disabled>
		             <option value="">-------------</option>
			<%         Vector E38HospCodeData_opt  = (new E38CancerHospCodeRFC()).getHospCode(Pernr);
			             for( int i = 0 ; i < E38HospCodeData_opt.size() ; i++ ) {
			                 E38CancerData data = (E38CancerData)E38HospCodeData_opt.get(i);
  ...
  <div class="listArea">
  	<div class="table">
            <table class="listTable">
				...
				%>
				          <td <%= inx>0 && inx<= E38CancerPossNumData_vt.size() && EXAM_POSS[inx-1].equals("X") ? "bgcolor=dc9146" : cur_day==inx ? "bgcolor=FFFECC" : "bgcolor=EFEEE8" %> height="19" align=center>
				<%
