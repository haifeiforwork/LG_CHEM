...
*   Program ID   : F28DeptCreditSeizor.jsp
*   Description  : 부서별 채권 압류자 조회를 위한 jsp 파일
*   Note         : 없음
*   Creation     :
...
function excelDown() {
    ...
    frm.action = "<%= WebUtil.ServletURL %>hris.F.F28DeptCreditSeizorSV";
    frm.target = "hidden";
    frm.submit();
}
...
<div class="listArea">
	<div class="listTop">
		<span class="listCnt"><<!-- 총 --><spring:message code='LABEL.F.FCOMMON.0006'/><%=DeptCreditSeizor_vt.size()%><spring:message code='LABEL.F.FCOMMON.0007'/><!-- 건 -->></span>
	...
	<div class="wideTable" style="border-top: 2px solid #c8294b;">
	    ...
	    <thead>
	        <tr>
				...
				<th><spring:message code="LABEL.F.F28.0011"/><!-- 압류일 --></th>
				<th><spring:message code="LABEL.F.F28.0012"/><!-- 압류금액 --></th>
				<th class="lastCol"><spring:message code="LABEL.F.F28.0013"/><!-- 사유 --></th>
          </tr>
        </thead>
<%
			String oldPer="";
...
%>
          <tr class="borderRow">
            ...
            <%} %>
            <td nowrap>&nbsp;<%= (data.BEGDA).equals("0000-00-00") ? "" : WebUtil.printDate(data.BEGDA) %>&nbsp;</td>
            <td nowrap>&nbsp;<%= WebUtil.printNumFormat(Double.parseDouble(data.CRED_AMNT)*100) %>&nbsp;</td>
            <td nowrap class="lastCol">&nbsp;<%= data.CRED_TEXT %>&nbsp;</td>
          </tr>
<%
	        } //end for...
%>
