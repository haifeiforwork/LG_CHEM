...
<%
    ...
    Vector DeptDutySchoolNote_vt  = (Vector)request.getAttribute("F08DeptDutySchoolNote_vt");          //내용
...
function goDetail(paramA, paramB, paramC, paramD, paramE, value){
    ...
    if( value!= "" && value!= "0"){
        frm.hdn_gubun.value  = "08";             //직무별/인정학력별
...
<%
            ...
            for( int k = 0; k < DeptDutySchoolTitle_vt.size()-1; k++ ){
                F08DeptDutySchoolTitleData titleData = (F08DeptDutySchoolTitleData)DeptDutySchoolTitle_vt.get(k);
%>
                    <th width="75" nowrap><%=titleData.STYPE_TEXT%></th>
<%
            }//end for
%>
...
<%
            ...
            for( int i = 0; i < DeptDutySchoolNote_vt.size(); i++ ){
                F08DeptDutySchoolNoteData data = (F08DeptDutySchoolNoteData)DeptDutySchoolNote_vt.get(i);
...
<%
                ...
                if ( setCnt == i && i < DeptDutySchoolNote_vt.size()-1) {
                    for( int inx = i; inx < DeptDutySchoolNote_vt.size(); inx++ ){
                        F08DeptDutySchoolNoteData dataTitl = (F08DeptDutySchoolNoteData)DeptDutySchoolNote_vt.get(inx);
                        ...
                        if( dataTitl.JIKK_TYPE.equals(noteCode) ){
                            ...
                            if( noteCnt>1 ){
%>
                    <td <%=titlClass%> rowspan="<%=noteCnt%>" ><%=data.JIKK_TEXT%></td>
<%
