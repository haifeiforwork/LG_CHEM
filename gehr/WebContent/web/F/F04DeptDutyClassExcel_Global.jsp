...
*   Program ID   : F04DeptDutyClassExcel_Global.jsp
*   Description  : 직무별/직급별 인원현황 Excel 저장을 위한 jsp 파일
*   Note         : 없음
*   Creation     :
...
<%
    ...
    /*----- Excel 파일 저장하기 --------------------------------------------------- */
    response.setHeader("Content-Disposition","attachment;filename=Job_Family_Level.xls");
    response.setContentType("application/vnd.ms-excel;charset=utf-8");
    /*----------------------------------------------------------------------------- */
    List list = (List)request.getAttribute("meta");
    HashMap meta = (HashMap)list.get(0);
    HashMap meta1 = (HashMap)list.get(1);
    F04DeptDutyClassTitleGlobalData total = new F04DeptDutyClassTitleGlobalData();
    AppUtil.initEntity(total,"0");
    if(F04DeptDutyClassTitle_vt.size() > 0)
  ...
  <tr>
    ...
    <td >
      <table width="100%" border="0" cellspacing="0" cellpadding="0" align="center">
        <tr>
          <td colspan="2" class="title02">* <spring:message code='LABEL.F.F04.0004'/><!-- Job Family -->/<spring:message code='LABEL.F.F04.0005'/><!-- Level --></td>
        </tr>
...
<table border="1" cellpadding="0" cellspacing="1" class="table02" align="left">
        <tr>
          ...
          <td rowspan="2" style="text-align:center" ><spring:message code='LABEL.F.F03.0002'/><!-- Pers. Area --></td>
          <td colspan="2" style="text-align:center" ><spring:message code='LABEL.F.F04.0003'/><!-- Job Catalog --></td>
          <%
        ...
        <tr>
          <td width="70" style="text-align:center"><spring:message code='LABEL.F.F04.0004'/><!-- Job Family --></td>
          <td width="70" style="text-align:center"><spring:message code='LABEL.F.F00.0012'/><!-- Job --></td>
          <%
        ...
        <tr>
        	...
        	<%
        		...
        		if(!entity.PBTXT.equals("TOTAL")){
        			if(!tmp1.equals(entity.JIKGT)){
        	%>
        	<td class="<%=entity.PBTXT.equals("TOTAL")?"td11":entity.JIKYT.equals("SUBTOTAL")?"td11_2":"td05"%>" nowrap="nowrap" rowspan="<%=meta1.get(entity.PBTXT + entity.JIKGT)%>"><%=entity.JIKGT %></td>
			<%
					}
			%>


        	<td class="<%=entity.PBTXT.equals("TOTAL")?"td11":entity.JIKYT.equals("SUBTOTAL")?"td11_2":"td05"%>" nowrap="nowrap"><%=entity.JIKYT %></td>
        	<%
        	}
        	if(Integer.parseInt(total.P01) > 0){
        	%>
        	<td class="<%=entity.PBTXT.equals("TOTAL")?"td11":entity.JIKYT.equals("SUBTOTAL")?"td11_2":"td05"%>" >
