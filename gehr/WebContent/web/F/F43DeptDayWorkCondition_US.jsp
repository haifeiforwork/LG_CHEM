...
/*   Description  		: 부서별 일간 근태 집계표 조회를 위한 jsp (USA)                													*/
/*   Note         		: 없음                                                        																		*/
		...
		<div class="table">
			...
			<thead>
				<tr>
					...
					%>
					<th class=<%=i==detailDataAll_vt.size()-1 ? "lastCol" :"" %>><%= Integer.parseInt(time.CAL_DATE.substring(8)) %></th>
					<%
						}
					%>
					<!-- column head -->
				...
				<tr class="<%=WebUtil.printOddRow(i)%>">
					...
					<%
						for (int j = 0; j < detailDataAll_vt.size(); j ++) {
							BetweenDateData time = (BetweenDateData)detailDataAll_vt.get(j);
					%>
					<td class=<%=j==detailDataAll_vt.size()-1 ? "lastCol" :"" %>><%= data.MAP.get(time.CAL_DATE) == null ? "" : data.MAP.get(time.CAL_DATE) %></td>
					<%
						}
					%>
				</tr>
		...
		<div class="table">
			...
			<thead>
	         	<tr>
					...
					<td>A&nbsp;:&nbsp;<!--Absence--><%=g.getMessage("LABEL.F.F43.0017")%></td>
					<td>O&nbsp;:&nbsp;<!--Others--><%=g.getMessage("LABEL.F.F43.0072")%></td>
					<td>S&nbsp;:&nbsp;<!--Sick--><%=g.getMessage("LABEL.F.F43.0079")%></td>
					<td class="lastCol">P&nbsp;:&nbsp;<!--Personal Time--><%=g.getMessage("LABEL.F.F43.0080")%></td>
				</tr>
				<tr class="borderRow">
					<td colspan="4" class="lastCol">V&nbsp;:&nbsp;<!--Vacation--><%=g.getMessage("LABEL.F.F43.0081")%></td>
				</tr>
				<tr class="borderRow">
	        	...
	        	<tr>
		            ...
		            <td class="lastCol" colspan="4">OT&nbsp;:&nbsp;<!--Overtime--><%=g.getMessage("LABEL.F.F43.0039")%></td>
				</tr>
			</table>
		</div>
	</div>
