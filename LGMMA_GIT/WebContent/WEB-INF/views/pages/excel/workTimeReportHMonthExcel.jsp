	            ...
	            <table border="1" cellpadding="0" cellspacing="1">
		            ...
		            <c:forEach var="tHeader" items="${T_HEADER}" varStatus="status">
		            	<tr <c:if test="${status.index eq 0}">style="background-color:#FFFF00;"</c:if>>
		            		...
		            		<c:choose>
		            			<c:when test="${status.index eq 0}"><td width="60" style="text-align:center;"><fmt:formatNumber value="${tHeader.MAXTM}" pattern="#,##0.00" /></td></c:when>
	            			...
	            			<td width="60" style="text-align:center;">
	            				<fmt:formatNumber var="decimalPoint" value="${(tHeader.RWKTM % 1) % 0.1}" />
         						<fmt:formatNumber value="${tHeader.RWKTM - (decimalPoint eq 0.1 ? 0 : decimalPoint)}" pattern="#,##0.00" />
	            			</td>
	            			...
	            			<td style="text-align:center;<c:if test="${convertAVRTM ge limitNumber}">background-color:#FFB6C1;</c:if>">
            					...
            					<fmt:formatNumber value="${tHeader.AVRTM - (decimalPoint eq 0.1 ? 0 : decimalPoint)}" pattern="#,##0.00" />
	            			</td>
		            	</tr>
		            </c:forEach>
		            </tbody>
				...
				<table border="1" cellpadding="0" cellspacing="1">
		            ...
		            <tbody>
		            	...
		            	<tr>
	            			...
	            			<td width="60" style="text-align:center;${bgColor}"><c:choose><c:when test="${body.NORTM eq '0'}">${body.NORTM}</c:when><c:otherwise><fmt:formatNumber value="${body.NORTM}" pattern="#,##0.00" /></c:otherwise></c:choose></td>
	            			<td width="60" style="text-align:center;${bgColor}"><c:choose><c:when test="${body.OVRTM eq '0'}">${body.OVRTM}</c:when><c:otherwise><fmt:formatNumber value="${body.OVRTM}" pattern="#,##0.00" /></c:otherwise></c:choose></td>
	            			<td width="60" style="text-align:center;${bgColor}"><c:choose><c:when test="${body.EDUTM eq '0'}">${body.EDUTM}</c:when><c:otherwise><fmt:formatNumber value="${body.EDUTM}" pattern="#,##0.00" /></c:otherwise></c:choose></td>
	            			<td width="60" style="text-align:center;${bgColor}"><c:choose><c:when test="${body.BRKTM eq '0'}">${body.BRKTM}</c:when><c:otherwise><fmt:formatNumber value="${body.BRKTM}" pattern="#,##0.00" /></c:otherwise></c:choose></td>
	            			<td width="60" style="text-align:center;${bgColor}"><c:choose><c:when test="${body.NWKTM eq '0'}">${body.NWKTM}</c:when><c:otherwise><fmt:formatNumber value="${body.NWKTM}" pattern="#,##0.00" /></c:otherwise></c:choose></td>
	            			<c:if test="${not empty T_NWKTYP}">
		            			<c:forEach var="nwktyp" items="${T_NWKTYP}" varStatus="status">
