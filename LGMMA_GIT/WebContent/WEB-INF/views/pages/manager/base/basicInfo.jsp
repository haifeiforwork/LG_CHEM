	...
	<div class="familyDetailArea">	
		...
		<div class="table pb30">
			<table class="tableGeneral">
				...
				<tbody>
				<tr>
					<th><label for="familyDetailName">성명</label></th>
					<td>
					...
					<th><label for="familyDetailSUBTY">가족유형</label></th>
					<td>
						<select id="familyDetailSUBTY" name="SUBTY" class="input03 readOnly" disabled>
						<option value="">------------</option>
				...
				<tr>
					<th><label for="familyDetailREGNO">주민등록번호</label></th>
					<td><input id="familyDetailREGNO" name="REGNO" type="text" onBlur="javascript:resno_chk('familyDetailREGNO');" class="readOnly" readOnly /></td>
					<th><label for="familyDetailKDSVH">관계</label></th>
					<td>
				...
				<tr>
					<th><label for="familyDetailFGBDT">생년월일</label></th>
					<td><input id="familyDetailFGBDT" name="FGBDT" type="text" class="readOnly" readOnly /></td>
				...
				<tr>
					...
					<th><label for="familyDetailFASAR">학력</label></th>
					<td>
	...
	<div id = "createLicense" class="tableArea">
		...
		<div class="table">
			...
			<tr>
				...
				<td>
					<input class="w200" type="text" id="licenseLicnName" name="LICN_NAME" vname="자격증" required />
		...
		<div class="tableComment pb30">
			...
			<ul>
				<li>자격증 종류가 미등록된 경우는 경영관리팀으로 자격증 종류 등록을 요청하신 후 재신청하시기 바랍니다.</li>
				<li>법정선임 대상 자격증은 환경안전팀으로 제출바랍니다.</li>
			</ul>
		</div>
		<div class="listArea">
				...
				<div class="table">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<input id="addrsTELNR" name="TELNR" type="text" class="w150" vname="전화번호" required/>
				...
				<table class="tableGeneral">
					...
					<tr>
						...
						<td>
							...
							<input id="createFamilyFNMHG" name="FNMHG" type="text" class="w100"/>
						</td>
					...
					<tr>
						...
						<td><input id="createFamilyREGNO" name="REGNO" type="text" onBlur="javascript:resno_chk('createFamilyREGNO');" /></td>
					...
					<tr>
						...
						<td><input id="createFamilyFGBDT" name="FGBDT" type="text"/></td>
						<th><label for="createFamilyFASEX">성별</label></th>
						<td>
							<input type="radio" name="FASEX" value=1 /> 남
							<input type="radio" name="FASEX" value=2 /> 여
						</td>
					</tr>
					<tr>
				...
				<div class="table">
					<table class="tableGeneral">
						...
						<tr>
							...
							<td><input id="supportFamilyREGNO" name="REGNO" type="text" readonly /></td>
...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#tab8").click(function(){
			$.ajax({
				type : "get",
				url : "/manager/base/c05FtestResultList.json",
				dataType : "json",
		...
		$("#createLicenseBtn").click(function(){
			...
			if(confirm("신청 하시겠습니까?")){
				...
				if( checkNullField("licenseForm") ) {
					jQuery.ajax({
						type : 'POST',
						url : '/manager/base/createLicense.json',
						cache : false,
						dataType : 'json',
						data : param,
						async :false,
		...
		$("#addrListGrid").jsGrid({
			...
			controller : {
				loadData : function() {
					...
					$.ajax({
						type : "GET",
						url : "/manager/base/getAddrList.json",
						dataType : "json"
					}).done(function(response) {
						if(response.success)
			...
			fields : [
				...
				{ name : "STRAS", title : "주소", align : "left", type : "text", width: "60%"
	...
	if(addrValidation()){
			...
			jQuery.ajax({
				type : 'POST',
				url : '/manager/base/updateAddr.json',
				cache : false,
				dataType : 'json',
	...
	$(function() {
		$("#contactListGrid").jsGrid({
			...
			paging: false,
			fields: [
				{ title: "자택전화", name: "TELNR", type: "text", align: "center", width: "34%" },
				{ title: "회사전화번호", name: "USRID", type: "text", align: "center", width: "33%" },
				{ title: "핸드폰번호", name: "MPHNID", type: "text", align: "center", width: "33%" }
			],
			controller : {
				loadData : function() {
					var d = $.Deferred();
					$.ajax({
						type : "GET",
						url : "/manager/base/getContactList.json",
						dataType : "json"
					}).done(function(response) {
						if(response.success)
							d.resolve(response.storeData);
	...
	// 연락처 등록/상세 popup
	var showContactView = function(dialogType, client) {
		$("#contactTELNR1").val(client.TELNR);
		$("#contactUSRID").val(client.USRID);
		$("#contactMPHNID").val(client.MPHNID);
		targetItem = client;
		callType = dialogType;
	...
	$("#contactSave").click(function(){
		saveContactClient(targetItem, callType === "Add");
		targetItem = null;
		callType = null;
	});
	...
	var saveContactClient = function(client, isNew) {
		if(contactValidation()){
			jQuery.ajax({
				type : 'POST',
				url : '/manager/base/updateContact.json',
				cache : false,
				dataType : 'json',
	...
	$(function() {
		$("#familyListGrid").jsGrid({
			...
			controller : {
				loadData : function() {
					...
					$.ajax({
						type : "GET",
						url : "/manager/base/getFamilyList.json",
						dataType : "json"
					}).done(function(response) {
						if(response.success)
							d.resolve(response.storeData);
		...
		var showFamilyDetailView = function(client) {
			$("familyDetailForm").each(function(){
 			...
 			$("#familyDetailLNMHG").val(client.LNMHG);
			$("#familyDetailFNMHG").val(client.FNMHG);
			$("#familyDetailSUBTY").val(client.SUBTY);
			$("#familyDetailREGNO").val(addResBar(client.REGNO));
			$("#familyDetailKDSVH").val(client.KDSVH);
			$("#familyDetailFGBDT").val(client.FGBDT);
			$('input:radio[name="FASEX"]');
			$('input:radio[name="FASEX"]:input[value='+client.FASEX+']').prop("checked", true);
			$("#familyDetailFGBOT").val(client.FGBOT);
			$("#familyDetailFASAR").val(client.FASAR);
			$("#familyDetailFGBLD").val(client.FGBLD);
			$("#familyDetailFASIN").val(client.FASIN);
			$("#familyDetailFANAT").val(client.FANAT);
			$("#familyDetailFAJOB").val(client.FAJOB);
	...
	var createFamilyClient = function() {
		if(createFamilyPopupCheck()){
			if(confirm("신청 하시겠습니까?")){
				...
				jQuery.ajax({
					type : 'POST',
					url : '/manager/base/createFamily.json',
					cache : false,
					dataType : 'json',
		...
		if( $("input:radio[name='familyChk']").is(":checked") ){
			if(createFamilyPopupCheck()){
				if(confirm("수정 하시겠습니까?")){
					...
					jQuery.ajax({
						type : 'POST',
						url : '/manager/base/updateFamily.json',
						cache : false,
						dataType : 'json',
	...
	$("#deleteFamilyClientBtn").click(function(){
		if( $("input:radio[name='familyChk']").is(":checked") ){
			if( confirm("정말 삭제하시겠습니까?") ) {
				jQuery.ajax({
					type : 'POST',
					url : '/manager/base/deleteFamily.json',
					cache : false,
					dataType : 'json',
					data : $('#familyDetailForm').serialize(),
					async :false,
	...
	var requestSupportClient = function() {
		if( requestSupportCheck() ) {
			...
			jQuery.ajax({
				type : 'POST',
				url : '/manager/base/requestSupportFamily.json',
				cache : false,
				dataType : 'json',
				data : param,
				async :false,
		...
		$("#rankChangeGrid").jsGrid({
			...
			controller : {
				loadData : function() {
					...
					$.ajax({
						type : "GET",
						url : "/manager/base/getRankChangeList.json",
						dataType : "json"
					}).done(function(response) {
						if(response.success)
							d.resolve(response.storeData);
		...
		$("#statusChangeGrid").jsGrid({
			...
			controller : {
				loadData : function() {
					...
					$.ajax({
						type : "GET",
						url : "/manager/base/getStatusChangeList.json",
						dataType : "json"
					}).done(function(response) {
						if(response.success)
							d.resolve(response.storeData);
		...
		$("#schoolGrid").jsGrid({
			...
			controller : {
				loadData : function() {
					...
					$.ajax({
						type : "GET",
						url : "/manager/base/getSchoolList.json",
						dataType : "json"
					}).done(function(response) {
						if(response.success)
							d.resolve(response.storeData);
		...
		$("#licenseGrid").jsGrid({
			...
			controller : {
				loadData : function() {
					...
					$.ajax({
						type : "GET",
						url : "/manager/base/getLicenseList.json",
						dataType : "json"
					}).done(function(response) {
						if(response.success)
							d.resolve(response.storeData);
		...
		$("#rewardGrid").jsGrid({
			...
			controller : {
				loadData : function() {
					...
					$.ajax({
						type : "GET",
						url : "/manager/base/getRewardList.json",
						dataType : "json"
					}).done(function(response) {
						if(response.success)
							d.resolve(response.storeData);
		...
		$("#punishGrid").jsGrid({
			...
			controller : {
				loadData : function() {
					...
					$.ajax({
						type : "GET",
						url : "/manager/base/getPunishList.json",
						dataType : "json"
					}).done(function(response) {
						if(response.success)
							d.resolve(response.storeData);
		...
		$("#careerGrid").jsGrid({
			...
			controller : {
				loadData : function() {
					...
					$.ajax({
						type : "GET",
						url : "/manager/base/getCareerList.json",
						dataType : "json"
					}).done(function(response) {
						if(response.success)
							d.resolve(response.storeData);
	...
	$(function() {
		$("#langTest0003Grid").jsGrid({
			...
			fields: [
				{ title: "검정일", name: "BEGDA", type: "text", align: "center", width: "16%" },
				{ title: "L/C", name: "LISN_SCOR", type: "text", align: "center", width: "17%" },
				{ title: "Structure", name: "STRU_SCOR", type: "text", align: "center", width: "17%" },
				{ title: "R/C", name: "READ_SCOR", type: "text", align: "center", width: "17%" },
				{ title: "Writing", name: "WRIT_SCOR", type: "text", align: "center", width: "16%" },
				{ title: "TOTAL", name: "LAST_FLAG", type: "text", align: "center", width: "17%" 
					,itemTemplate: function(value, item) {
						return "<font color='" + (value == "Y" ? "#FF0000" : "#000000") + "' >"  + item.TOTL_SCOR + "</font>";
					}
				}
	...
	$(function() {
		$("#langTest0008Grid").jsGrid({
			...
			fields: [
				{ title: "검정일", name: "BEGDA", type: "text", align: "center", width: "10%" },
				{ title: "청력", name: "HEAR_SCOR", type: "text", align: "center", width: "10%" },
				{ title: "어법", name: "EXPR_SCOR", type: "text", align: "center", width: "10%" },
				{ title: "독해", name: "UNDR_SCOR", type: "text", align: "center", width: "14%" },
				{ title: "작문", name: "COMP_SCOR", type: "text", align: "center", width: "14%" },
				{ title: "구술", name: "ORAL_SCOR", type: "text", align: "center", width: "14%" },
				{ title: "TOTAL", name: "TOTL_SCOR", type: "text", align: "center", width: "14%" },
				{ title: "등급", name: "LAST_FLAG", type: "text", align: "center", width: "14%" 
					,itemTemplate: function(value, item) {
						return "<font color='" + (value == "Y" ? "#FF0000" : "#000000") + "' >"  + item.LANG_LEVL + "</font>";
					}
				}
	...
	$(function() {
		$("#langTest0010Grid").jsGrid({
			...
			fields: [
				...
				{ title: "등급", name: "LAST_FLAG", type: "text", align: "center", width: "65%"
					,itemTemplate: function(value, item) { 
						return value + " <font color='" + (value == "Y" ? "#FF0000" : "#000000") + "' >"  + item.TSPE_SCOR + "</font>";
					}
				}
			]
		});
		...
		$("#licensePopupGrid").jsGrid({
			...
			controller : {
				loadData : function() {
					...
					$.ajax({
						type : "post",
						url : "/manager/base/getLicensePopupList.json",
						dataType : "json",
