...
function makeSwfSingleUpload(){
	...
	flashStr += "<embed src='../web-resource/images/flash/single_upload.swf' width='"+flash_width+"' height='50' quality='high'";
...
function makeSwfMultiUpload(){
	...
	var flashStr = "<object classid='clsid:d27cdb6e-ae6d-11cf-96b8-444553540000'";
	flashStr += "codebase='http://fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=8,0,0,0'";
	flashStr += "width='"+flash_width+"' height='"+parseInt(list_rows*20+20,20)+"' align='middle' id='"+movie_id+"' method='multi_upload'>";
	flashStr += "<param name='allowScriptAccess' value='sameDomain' />";
	flashStr += "<param name='movie' value='multi_upload.swf' />";
	flashStr += "<param name='quality' value='high' />";
	flashStr += "<param name='bgcolor' value='#ffffff' />";
	flashStr += "<param name='flashvars' value='"+flashvars+"' />";
	flashStr += "<embed id='multi_upload' src='../web-resource/images/flash/multi_upload.swf' width='"+flash_width+"' height='"+parseInt(list_rows*20+80,10)+"' quality='high' wmode='transparent'";//@2015  연말정산  높이 수정
	flashStr += "bgcolor='#ffffff' name='"+movie_id+"' align='middle' allowScriptAccess='sameDomain' type='application/x-shockwave-flash'";
	flashStr += "pluginspage='http://www.macromedia.com/go/getflashplayer' flashvars='"+flashvars+"' />";
	flashStr += "</object>";
	document.write(flashStr);
...
function swfUploadComplete(){
	...
	if(arrMovie.length>arr_mov){
		...
		var pdfUpErrMsg = getCookie("pdfUpErrMsg");
		
		if(pdfUpErrMsg!=""){
			alert(decodeURIComponent(pdfUpErrMsg).replace(/\+/g, ' '));
		}
		
		pdfTransXml(pdfUpErrMsg);
	}
}
